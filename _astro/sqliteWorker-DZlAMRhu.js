(function(){"use strict";let N=null,t=null,K=!1,V="memory";const ue=(typeof{url:self.location.href}<"u","/psm/"+"vendor/sqlite/"),X="gps_active_point";function H(e,a=50,s=500){const n=Number(e);return!Number.isFinite(n)||n<=0?a:Math.min(Math.max(1,Math.floor(n)),s)}function Ae(e){if(typeof e!="string")return null;try{return JSON.parse(e)}catch(a){return console.warn("Failed to parse payload JSON",a),null}}function oe(e=t){e&&e.exec(`
  ...
  `);const s=[{name:"dt_label",type:"TEXT"},{name:"language_label",type:"TEXT"},{name:"authority",type:"TEXT"},{name:"name_de",type:"TEXT"},{name:"name_en",type:"TEXT"},{name:"name_la",type:"TEXT"}];for(const n of s)L(e,"lookup_eppo_codes",n.name)||e.exec(`ALTER TABLE lookup_eppo_codes ADD COLUMN ${n.name} ${n.type}`)}function v(e=t){e&&e.exec(`
  ...
  `)}function Ie(){return typeof crypto<"u"&&typeof crypto.randomUUID=="function"?crypto.randomUUID():`archive_${Date.now()}_${Math.random().toString(16).slice(2)}`}function ce(e){if(!e||typeof e!="object")return null;const a=e.id?String(e.id):Ie(),s=e.archivedAt||e.archived_at||new Date().toISOString(),n=e.startDate||e.start_date||null,i=e.endDate||e.end_date||null,r=Number(e.entryCount??e.entry_count??0)||0,l=e.fileName||e.file_name||"",u=e.storageHint||e.storage_hint||null,o=e.note??e.note_text??null;return{id:a,archivedAt:s,startDate:n,endDate:i,entryCount:r,fileName:l,storageHint:u,note:o,metadata:{...e,id:a,archivedAt:s,startDate:n,endDate:i,entryCount:r,fileName:l,storageHint:u,note:o}}}function Y(e){return{id:e.id,archivedAt:e.archivedAt,startDate:e.startDate??null,endDate:e.endDate??null,entryCount:e.entryCount??0,fileName:e.fileName??"",storageHint:e.storageHint??null,note:e.note??null}}function te(e=[],a=t){if(q(a),a.exec("DELETE FROM archive_logs"),!Array.isArray(e)||!e.length)return[];const s=a.prepare(`INSERT OR REPLACE INTO archive_logs
      (id, archived_at, start_date, end_date, entry_count, file_name, storage_hint, note, metadata_json)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`),n=[];for(const i of e){const r=ce(i);r&&(s.bind([r.id,r.archivedAt,r.startDate,r.endDate,r.entryCount,r.fileName,r.storageHint,r.note,JSON.stringify(r.metadata||i||{})]).step(),s.reset(),n.push(Y(r)))}return s.finalize(),n}function ne(e=t){q(e);const a=[];return e.exec({sql:`SELECT id, archived_at, start_date, end_date, entry_count, file_name, storage_hint, note, metadata_json
          FROM archive_logs
          ORDER BY datetime(archived_at) DESC, id DESC`,rowMode:"object",callback:s=>{const n=ae(s);n&&a.push(n)}}),a}function ae(e){if(!e)return null;const a=e.metadata_json?Ae(e.metadata_json):null,s=a&&typeof a=="object"?{...a}:{};return s.id=String(e.id),s.archivedAt=e.archived_at||s.archivedAt||new Date().toISOString(),s.startDate=e.start_date??s.startDate??null,s.endDate=e.end_date??s.endDate??null,s.entryCount=Number(e.entry_count??s.entryCount??0)||0,s.fileName=e.file_name||s.fileName||"",s.storageHint=e.storage_hint??s.storageHint??null,s.note=e.note??s.note??null,s}function de(e=t){const a=ne(e).map(s=>Y(s));x("archives",JSON.stringify({logs:a}),e)}function Oe(){return typeof crypto<"u"&&typeof crypto.randomUUID=="function"?crypto.randomUUID():`gps_${Date.now()}_${Math.random().toString(16).slice(2)}`}function ie(e){return e?{id:String(e.id),name:e.name!=null?String(e.name):"",description:e.description??null,latitude:Number(e.latitude),longitude:Number(e.longitude),source:e.source??null,created_at:e.created_at||new Date().toISOString(),updated_at:e.updated_at||new Date().toISOString(),nutzflaeche_qm:e.nutzflaeche_qm!=null?Number(e.nutzflaeche_qm):null,kind:e.kind??null}:null}function ke(e){if(!t||!e)return null;let a=null;return t.exec({sql:`SELECT id, name, description, latitude, longitude, source, created_at, updated_at, nutzflaeche_qm, kind
           FROM gps_points
           WHERE id = ?
          ...
          FROM gps_points
          ORDER BY datetime(updated_at) DESC`,rowMode:"object",callback:s=>{e.push(ie(s))}});const a=U(X);return{rows:e,activePointId:a||null}}async function Me(e={}){if(!t)throw new Error("Database not initialized");v();const{cursor:a=null,limit:s,pageSize:n,search:i,includeTotal:r=!1}=e||{},l=H(n??s,100,500),u=typeof i=="string"?i.trim():"",o=[],c=[];if(u){o.push("(LOWER(name) LIKE LOWER(?) OR LOWER(COALESCE(description, '')) LIKE LOWER(?))");const m=`%${u}%`;c.push(m,m)}const g=[...o],_=[...c];if(a&&a.updatedAt){const m=a.id!=null?String(a.id):"",E=a.updatedAt;g.push(`(
  ...
  `)}function he(e){return e?{id:String(e.id),flaecheTyp:e.flaeche_typ??null,flaecheId:e.flaeche_id!=null?String(e.flaeche_id):null,kultur:e.kultur??null,eppoCode:e.eppo_code??null,status:e.status??"geplant",pflanzDatum:e.pflanz_datum??null,ernteDatum:e.ernte_datum??null,ernteVon:e.ernte_von??null,ernteBis:e.ernte_bis??e.ernte_datum??null,color:e.color??null,notes:e.notes??null,aussaatDatum:e.aussaat_datum??null,kulturStammId:e.kultur_stamm_id??null,menge:e.menge!=null?Number(e.menge):null,einheit:e.einheit??null,satzGruppe:e.satz_gruppe??null,createdAt:e.created_at||null,updatedAt:e.updated_at||null}:null}function pe(e){return e?{id:String(e.id),name:e.name??"",familie:e.familie??null,anbauMethode:e.anbau_methode??null,anzuchtTage:e.anzucht_tage!=null?Number(e.anzucht_tage):null,kulturTage:e.kultur_tage!=null?Number(e.kultur_tage):null,ernteTage:e.ernte_tage!=null?Number(e.ernte_tage):null,reihenAbstandCm:e.reihen_abstand_cm!=null?Number(e.reihen_abstand_cm):null,pflanzAbstandCm:e.pflanz_abstand_cm!=null?Number(e.pflanz_abstand_cm):null,bioTyp:e.bio_typ??null,color:e.color??null,eppoCode:e.eppo_code??null,notes:e.notes??null,createdAt:e.created_at||null,updatedAt:e.updated_at||null}:null}function ge(e){return e?{id:String(e.id),flaecheTyp:e.flaeche_typ??null,flaecheId:e.flaeche_id!=null?String(e.flaeche_id):null,anbauId:e.anbau_id??null,art:e.art??"sonstiges",status:e.status??"geplant",planDatum:e.plan_datum??null,erledigtDatum:e.erledigt_datum??null,menge:e.menge!=null?Number(e.menge):null,einheit:e.einheit??null,mittel:e.mittel??null,historyId:e.history_id!=null?Number(e.history_id):null,notes:e.notes??null,createdAt:e.created_at||null,updatedAt:e.updated_at||null}:null}async function We(e={}){if(!t)throw new Error("Database not initialized");P();const a=[],s=[];e.flaecheTyp&&(a.push("flaeche_typ = ?"),s.push(String(e.flaecheTyp))),e.flaecheId&&(a.push("flaeche_id = ?"),s.push(String(e.flaecheId))),e.status&&(a.push("status = ?"),s.push(String(e.status)));const n="SELECT * FROM anbau_kultur"+(a.length?" WHERE "+a.join(" AND "):"")+" ORDER BY COALESCE(pflanz_datum, created_at) ASC",i=[];return t.exec({sql:n,bind:s,rowMode:"object",callback:r=>i.push(he(r))}),{rows:i}}async function je(e={}){if(!t)throw new Error("Database not initialized");P();const a=String(e.id||F()),s=new Date().toISOString(),n=[String(e.flaecheTyp||""),String(e.flaecheId||""),e.kultur!=null?String(e.kultur):null,e.eppoCode!=null?String(e.eppoCode):null,String(e.status||"geplant"),e.pflanzDatum!=null?String(e.pflanzDatum):null,e.ernteDatum!=null?String(e.ernteDatum):null,e.ernteVon!=null?String(e.ernteVon):null,e.ernteBis!=null?String(e.ernteBis):null,e.color!=null?String(e.color):null,e.notes!=null?String(e.notes):null,e.aussaatDatum!=null?String(e.aussaatDatum):null,e.kulturStammId!=null?String(e.kulturStammId):null,e.menge!=null&&e.menge!==""?Number(e.menge):null,e.einheit!=null?String(e.einheit):null,e.satzGruppe!=null?String(e.satzGruppe):null];if(t.selectValue("SELECT 1 FROM anbau_kultur WHERE id = ? LIMIT 1",[a])){const l=t.prepare("UPDATE anbau_kultur SET flaeche_typ=?, flaeche_id=?, kultur=?, eppo_code=?, status=?, pflanz_datum=?, ernte_datum=?, ernte_von=?, ernte_bis=?, color=?, notes=?, aussaat_datum=?, kultur_stamm_id=?, menge=?, einheit=?, satz_gruppe=?, updated_at=? WHERE id=?");l.bind([...n,s,a]),l.step(),l.finalize()}else{const l=t.prepare("INSERT INTO anbau_kultur (flaeche_typ, flaeche_id, kultur, eppo_code, status, pflanz_datum, ernte_datum, ernte_von, ernte_bis, color, notes, aussaat_datum, kultur_stamm_id, menge, einheit, satz_gruppe, created_at, updated_at, id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");l.bind([...n,s,s,a]),l.step(),l.finalize()}let r=null;return t.exec({sql:"SELECT * FROM anbau_kultur WHERE id = ? LIMIT 1",bind:[a],rowMode:"object",callback:l=>{r||(r=he(l))}}),r}async function Ke(e={}){if(!t)throw new Error("Database not initialized");P(),B();const a=String(e.id??"").trim();if(!a)throw new Error("ID fehlt.");const s=t.prepare("UPDATE massnahme SET anbau_id = NULL WHERE anbau_id = ?");s.bind([a]),s.step(),s.finalize();const n=t.prepare("DELETE FROM anbau_kultur WHERE id = ?");return n.bind([a]),n.step(),n.finalize(),{success:!0}}async function Ve(){if(!t)throw new Error("Database not initialized");Q(),_e();const e=[];return t.exec({sql:"SELECT * FROM kultur_stamm ORDER BY name COLLATE NOCASE ASC",rowMode:"object",callback:a=>e.push(pe(a))}),{rows:e}}async function $e(e={}){if(!t)throw new Error("Database not initialized");Q();const a=String(e.id||F()),s=new Date().toISOString(),n=u=>u!=null&&u!==""&&Number.isFinite(Number(u))?Math.round(Number(u)):null,i=[String(e.name||"").trim(),e.familie!=null?String(e.familie):null,e.anbauMethode!=null?String(e.anbauMethode):null,n(e.anzuchtTage),n(e.kulturTage),n(e.ernteTage),n(e.reihenAbstandCm),n(e.pflanzAbstandCm),e.bioTyp!=null?String(e.bioTyp):null,e.color!=null?String(e.color):null,e.eppoCode!=null?String(e.eppoCode):null,e.notes!=null?String(e.notes):null];if(t.selectValue("SELECT 1 FROM kultur_stamm WHERE id = ? LIMIT 1",[a])){const u=t.prepare("UPDATE kultur_stamm SET name=?, familie=?, anbau_methode=?, anzucht_tage=?, kultur_tage=?, ernte_tage=?, reihen_abstand_cm=?, pflanz_abstand_cm=?, bio_typ=?, color=?, eppo_code=?, notes=?, updated_at=? WHERE id=?");u.bind([...i,s,a]),u.step(),u.finalize()}else{const u=t.prepare("INSERT INTO kultur_stamm (name, familie, anbau_methode, anzucht_tage, kultur_tage, ernte_tage, reihen_abstand_cm, pflanz_abstand_cm, bio_typ, color, eppo_code, notes, created_at, updated_at, id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");u.bind([...i,s,s,a]),u.step(),u.finalize()}let l=null;return t.exec({sql:"SELECT * FROM kultur_stamm WHERE id = ? LIMIT 1",bind:[a],rowMode:"object",callback:u=>{l||(l=pe(u))}}),l}async function Ye(e={}){if(!t)throw new Error("Database not initialized");Q();const a=String(e.id??"").trim();if(!a)throw new Error("ID fehlt.");const s=t.prepare("DELETE FROM kultur_stamm WHERE id = ?");return s.bind([a]),s.step(),s.finalize(),{success:!0}}async function Je(e={}){if(!t)throw new Error("Database not initialized");B();const a=[],s=[];e.flaecheTyp&&(a.push("flaeche_typ = ?"),s.push(String(e.flaecheTyp))),e.flaecheId&&(a.push("flaeche_id = ?"),s.push(String(e.flaecheId))),e.anbauId&&(a.push("anbau_id = ?"),s.push(String(e.anbauId))),e.art&&(a.push("art = ?"),s.push(String(e.art))),e.status&&(a.push("status = ?"),s.push(String(e.status))),e.from&&(a.push("COALESCE(plan_datum, erledigt_datum) >= ?"),s.push(String(e.from))),e.to&&(a.push("COALESCE(plan_datum, erledigt_datum) <= ?"),s.push(String(e.to)));const n="SELECT * FROM massnahme"+(a.length?" WHERE "+a.join(" AND "):"")+" ORDER BY COALESCE(plan_datum, erledigt_datum, created_at) ASC",i=[];return t.exec({sql:n,bind:s,rowMode:"object",callback:r=>i.push(ge(r))}),{rows:i}}async function Qe(e={}){if(!t)throw new Error("Database not initialized");B();const a=String(e.id||F()),s=new Date().toISOString(),n=u=>u!=null&&Number.isFinite(Number(u))?Number(u):null,i=[String(e.flaecheTyp||""),String(e.flaecheId||""),e.anbauId!=null?String(e.anbauId):null,String(e.art||"sonstiges"),String(e.status||"geplant"),e.planDatum!=null?String(e.planDatum):null,e.erledigtDatum!=null?String(e.erledigtDatum):null,n(e.menge),e.einheit!=null?String(e.einheit):null,e.mittel!=null?String(e.mittel):null,n(e.historyId),e.notes!=null?String(e.notes):null];if(t.selectValue("SELECT 1 FROM massnahme WHERE id = ? LIMIT 1",[a])){const u=t.prepare("UPDATE massnahme SET flaeche_typ=?, flaeche_id=?, anbau_id=?, art=?, status=?, plan_datum=?, erledigt_datum=?, menge=?, einheit=?, mittel=?, history_id=?, notes=?, updated_at=? WHERE id=?");u.bind([...i,s,a]),u.step(),u.finalize()}else{const u=t.prepare("INSERT INTO massnahme (flaeche_typ, flaeche_id, anbau_id, art, status, plan_datum, erledigt_datum, menge, einheit, mittel, history_id, notes, created_at, updated_at, id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");u.bind([...i,s,s,a]),u.step(),u.finalize()}let l=null;return t.exec({sql:"SELECT * FROM massnahme WHERE id = ? LIMIT 1",bind:[a],rowMode:"object",callback:u=>{l||(l=ge(u))}}),l}async function Ze(e={}){if(!t)throw new Error("Database not initialized");B();const a=String(e.id??"").trim();if(!a)throw new Error("ID fehlt.");const s=t.prepare("DELETE FROM massnahme WHERE id = ?");return s.bind([a]),s.step(),s.finalize(),{success:!0}}async function et(){if(!t)throw new Error("Database not initialized");B();const e=new Set;t.exec({sql:"SELECT DISTINCT history_id AS h FROM massnahme WHERE history_id IS NOT NULL",rowMode:"object",callback:u=>{u.h!=null&&e.add(Number(u.h))}});const a=new Set;try{t.exec({sql:"SELECT id FROM gps_points WHERE kind = 'gewaechshaus'",rowMode:"object",callback:u=>a.add(String(u.id))})}catch{}const s=new Map;try{t.exec({sql:"SELECT id, standort_id FROM ackerflaechen WHERE standort_id IS NOT NULL",rowMode:"object",callback:u=>{const o=String(u.standort_id);s.has(o)||s.set(o,[]),s.get(o).push(String(u.id))}})}catch{}const n=[];try{t.exec({sql:"SELECT id, date_iso, kultur, gps_point_id FROM history WHERE gps_point_id IS NOT NULL ORDER BY id ASC",rowMode:"object",callback:u=>n.push(u)})}catch{return{imported:0,skipped:0,total:0}}let i=0,r=0;const l=new Date().toISOString();for(const u of n){const o=Number(u.id);if(e.has(o))continue;const c=u.gps_point_id!=null?String(u.gps_point_id):"";let g=null,_=null;if(c&&a.has(c)?(g="haus",_=c):c&&s.has(c)&&s.get(c).length===1&&(g="acker",_=s.get(c)[0]),!g){r++;continue}const p=F(),b=t.prepare("INSERT INTO massnahme (flaeche_typ, flaeche_id, anbau_id, art, status, plan_datum, erledigt_datum, menge, einheit, mittel, history_id, notes, created_at, updated_at, id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");b.bind([g,_,null,"chemisch_psm","erledigt",null,u.date_iso||null,null,null,null,o,u.kultur!=null?String(u.kultur):null,l,l,p]),b.step(),b.finalize(),e.add(o),i++}return{imported:i,skipped:r,total:n.length}}function G(e=t){e&&e.exec(`
  ...
  `)}function Te(e){return e?{id:String(e.id),kennr:e.kennr??null,mittelName:e.mittel_name!=null?String(e.mittel_name):"",wirkstoff:e.wirkstoff??null,typ:e.typ||"zugang",menge:e.menge!=null?Number(e.menge):0,einheit:e.einheit??null,datum:e.datum??null,charge:e.charge??null,ablauf:e.ablauf??null,lieferant:e.lieferant??null,preis:e.preis!=null?Number(e.preis):null,bemerkung:e.bemerkung??null,createdAt:e.created_at||null,updatedAt:e.updated_at||null}:null}async function tt(e={}){if(!t)throw new Error("Database not initialized");G();const a=e&&e.kennr!=null?String(e.kennr):null,s=[];return t.exec({sql:`SELECT * FROM lager_bewegungen ${a?"WHERE kennr = ?":""}
          ORDER BY datetime(COALESCE(datum, created_at)) DESC, created_at DESC`,bind:a?[a]:[],rowMode:"object",callback:n=>s.push(Te(n))}),{rows:s}}async function nt(e={}){if(!t)throw new Error("Database not initialized");G();const a=String(e.id||F()),s=String(e.mittelName??e.mittel_name??"").trim();if(!s)throw new Error("Mittelname ist erforderlich.");const n=Number(e.menge);if(!Number.isFinite(n))throw new Error("Menge ist ungültig.");const i=new Date().toISOString(),r={kennr:e.kennr!=null?String(e.kennr):null,wirkstoff:e.wirkstoff!=null?String(e.wirkstoff):null,typ:e.typ!=null?String(e.typ):"zugang",einheit:e.einheit!=null?String(e.einheit):null,datum:e.datum!=null?String(e.datum):i.slice(0,10),charge:e.charge!=null?String(e.charge):null,ablauf:e.ablauf!=null?String(e.ablauf):null,lieferant:e.lieferant!=null?String(e.lieferant):null,preis:e.preis!=null&&Number.isFinite(Number(e.preis))?Number(e.preis):null,bemerkung:e.bemerkung!=null?String(e.bemerkung):null};if(t.selectValue("SELECT 1 FROM lager_bewegungen WHERE id = ? LIMIT 1",[a])){const o=t.prepare("UPDATE lager_bewegungen SET kennr=?, mittel_name=?, wirkstoff=?, typ=?, menge=?, einheit=?, datum=?, charge=?, ablauf=?, lieferant=?, preis=?, bemerkung=?, updated_at=? WHERE id=?");o.bind([r.kennr,s,r.wirkstoff,r.typ,n,r.einheit,r.datum,r.charge,r.ablauf,r.lieferant,r.preis,r.bemerkung,i,a]),o.step(),o.finalize()}else{const o=t.prepare("INSERT INTO lager_bewegungen (id, kennr, mittel_name, wirkstoff, typ, menge, einheit, datum, charge, ablauf, lieferant, preis, bemerkung, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");o.bind([a,r.kennr,s,r.wirkstoff,r.typ,n,r.einheit,r.datum,r.charge,r.ablauf,r.lieferant,r.preis,r.bemerkung,i,i]),o.step(),o.finalize()}let u=null;return t.exec({sql:"SELECT * FROM lager_bewegungen WHERE id = ? LIMIT 1",bind:[a],rowMode:"object",callback:o=>{u||(u=Te(o))}}),u}async function at(e={}){if(!t)throw new Error("Database not initialized");G();const a=String(e.id??"").trim();if(!a)throw new Error("ID fehlt.");const s=t.prepare("DELETE FROM lager_bewegungen WHERE id = ?");return s.bind([a]),s.step(),s.finalize(),{success:!0}}async function it(){if(!t)throw new Error("Database not initialized");G();const e=new Map,a=(i,r)=>i&&String(i).trim()||`name:${String(r||"").toLowerCase().trim()}`,s=(i,r)=>{const l=a(i,r);return e.has(l)||e.set(l,{kennr:i||null,name:r||"",einheit:null,wirkstoff:null,zugang:0,verbraucht:0,bestand:0,zulEnde:null,naechsterAblauf:null,anwendungen:0}),e.get(l)};t.exec({sql:`SELECT kennr, mittel_name, einheit, wirkstoff,
          ...
          FROM history_items GROUP BY zulassungsnummer, medium_name`,rowMode:"object",callback:i=>{const r=s(i.kennr,i.medium_name);r.verbraucht+=Number(i.verbraucht)||0,r.anwendungen+=Number(i.n)||0,i.medium_unit&&!r.einheit&&(r.einheit=i.medium_unit),i.wirkstoff&&!r.wirkstoff&&(r.wirkstoff=i.wirkstoff)}});const n=Array.from(e.values());for(const i of n)i.bestand=i.zugang-i.verbraucht;return n.sort((i,r)=>(r.verbraucht||0)-(i.verbraucht||0)),{rows:n}}function W(e=t){e&&e.exec(`
  ...
  `)}function Z(e){return typeof crypto<"u"&&typeof crypto.randomUUID=="function"?crypto.randomUUID():`${e}_${Date.now()}_${Math.random().toString(16).slice(2)}`}async function st(e={}){if(!t)throw new Error("Database not initialized");W();const{favoritesOnly:a=!1,limit:s=100}=e,n=a?"WHERE is_favorite = 1":"",i=[];return t.exec({sql:`SELECT id, code, name, language, dtcode, usage_count, is_favorite, created_at, updated_at
          FROM saved_eppo_codes
          ${n}
          ORDER BY is_favorite DESC, usage_count DESC, name ASC
          LIMIT ?`,bind:[s],rowMode:"object",callback:r=>i.push({id:r.id,code:r.code,name:r.name,language:r.language,dtcode:r.dtcode,usageCount:r.usage_count,isFavorite:!!r.is_favorite,createdAt:r.created_at,updatedAt:r.updated_at})}),{items:i}}async function rt(e={}){if(!t)throw new Error("Database not initialized");W();const a=String(e.code??"").trim(),s=String(e.name??"").trim();if(!a||!s)throw new Error("EPPO-Code und Name sind erforderlich.");const n=e.id||Z("eppo"),i=e.language||null,r=e.dtcode||null,l=e.isFavorite?1:0,u=new Date().toISOString(),o=t.selectValue("SELECT id FROM saved_eppo_codes WHERE code = ? LIMIT 1",[a]);if(o){const c=t.prepare(`
      UPDATE saved_eppo_codes 
      SET name = ?, language = ?, dtcode = ?, is_favorite = ?, updated_at = ?
      WHERE code = ?
    `);return c.bind([s,i,r,l,u,a]),c.step(),c.finalize(),{id:o,code:a,name:s,updated:!0}}else{const c=t.prepare(`
      INSERT INTO saved_eppo_codes (id, code, name, language, dtcode, usage_count, is_favorite, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?)
    `);return c.bind([n,a,s,i,r,l,u,u]),c.step(),c.finalize(),{id:n,code:a,name:s,created:!0}}}async function lt(e={}){if(!t)throw new Error("Database not initialized");W();const a=String(e.id??"").trim();if(!a)throw new Error("ID erforderlich.");const s=t.prepare("DELETE FROM saved_eppo_codes WHERE id = ?");return s.bind([a]),s.step(),s.finalize(),{success:!0}}async function ut(e={}){if(!t)throw new Error("Database not initialized");W();const a=String(e.code??"").trim(),s=String(e.name??"").trim();if(!a)return{success:!1};const n=new Date().toISOString();if(t.selectValue("SELECT id FROM saved_eppo_codes WHERE code = ? LIMIT 1",[a])){const r=t.prepare(`
      UPDATE saved_eppo_codes SET usage_count = usage_count + 1, updated_at = ? WHERE code = ?
    `);r.bind([n,a]),r.step(),r.finalize()}else if(s){const r=Z("eppo"),l=t.prepare(`
      INSERT INTO saved_eppo_codes (id, code, name, language, dtcode, usage_count, is_favorite, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 1, 0, ?, ?)
    `);l.bind([r,a,s,e.language||null,e.dtcode||null,n,n]),l.step(),l.finalize()}return{success:!0}}async function ot(e={}){if(!t)throw new Error("Database not initialized");j();const{favoritesOnly:a=!1,limit:s=100}=e,n=a?"WHERE is_favorite = 1":"",i=[];return t.exec({sql:`SELECT id, code, label, principal_stage, secondary_stage, usage_count, is_favorite, created_at, updated_at
          FROM saved_bbch_stages
          ${n}
          ORDER BY is_favorite DESC, usage_count DESC, code ASC
          LIMIT ?`,bind:[s],rowMode:"object",callback:r=>i.push({id:r.id,code:r.code,label:r.label,principalStage:r.principal_stage,secondaryStage:r.secondary_stage,usageCount:r.usage_count,isFavorite:!!r.is_favorite,createdAt:r.created_at,updatedAt:r.updated_at})}),{items:i}}async function ct(e={}){if(!t)throw new Error("Database not initialized");j();const a=String(e.code??"").trim(),s=String(e.label??"").trim();if(!a||!s)throw new Error("BBCH-Code und Bezeichnung sind erforderlich.");const n=e.id||Z("bbch"),i=e.principalStage!=null?Number(e.principalStage):null,r=e.secondaryStage!=null?Number(e.secondaryStage):null,l=e.isFavorite?1:0,u=new Date().toISOString(),o=t.selectValue("SELECT id FROM saved_bbch_stages WHERE code = ? LIMIT 1",[a]);if(o){const c=t.prepare(`
      UPDATE saved_bbch_stages 
      SET label = ?, principal_stage = ?, secondary_stage = ?, is_favorite = ?, updated_at = ?
      WHERE code = ?
    `);return c.bind([s,i,r,l,u,a]),c.step(),c.finalize(),{id:o,code:a,label:s,updated:!0}}else{const c=t.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?)
    `);return c.bind([n,a,s,i,r,l,u,u]),c.step(),c.finalize(),{id:n,code:a,label:s,created:!0}}}async function dt(e={}){if(!t)throw new Error("Database not initialized");j();const a=String(e.id??"").trim();if(!a)throw new Error("ID erforderlich.");const s=t.prepare("DELETE FROM saved_bbch_stages WHERE id = ?");return s.bind([a]),s.step(),s.finalize(),{success:!0}}async function mt(e={}){if(!t)throw new Error("Database not initialized");j();const a=String(e.code??"").trim(),s=String(e.label??"").trim();if(!a)return{success:!1};const n=new Date().toISOString();if(t.selectValue("SELECT id FROM saved_bbch_stages WHERE code = ? LIMIT 1",[a])){const r=t.prepare(`
      UPDATE saved_bbch_stages SET usage_count = usage_count + 1, updated_at = ? WHERE code = ?
    `);r.bind([n,a]),r.step(),r.finalize()}else if(s){const r=Z("bbch"),l=t.prepare(`
      INSERT INTO saved_bbch_stages (id, code, label, principal_stage, secondary_stage, usage_count, is_favorite, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 1, 0, ?, ?)
    `);l.bind([r,a,s,e.principalStage||null,e.secondaryStage||null,n,n]),l.step(),l.finalize()}return{success:!0}}async function Et(e={}){if(!t)throw new Error("Database not initialized");W();const a=e.limit||10,s=[];return t.exec({sql:`SELECT code, name, language, dtcode, usage_count, is_favorite
          FROM saved_eppo_codes
          WHERE usage_count > 0
          ORDER BY usage_count DESC, name ASC
          LIMIT ?`,bind:[a],rowMode:"object",callback:n=>s.push({code:n.code,name:n.name,language:n.language,dtcode:n.dtcode,usageCount:n.usage_count,isFavorite:!!n.is_favorite})}),{items:s}}async function _t(e={}){if(!t)throw new Error("Database not initialized");j();const a=e.limit||10,s=[];return t.exec({sql:`SELECT code, label, principal_stage, secondary_stage, usage_count, is_favorite
          FROM saved_bbch_stages
          WHERE usage_count > 0
          ORDER BY usage_count DESC, code ASC
          LIMIT ?`,bind:[a],rowMode:"object",callback:n=>s.push({code:n.code,label:n.label,principalStage:n.principal_stage,secondaryStage:n.secondary_stage,usageCount:n.usage_count,isFavorite:!!n.is_favorite})}),{items:s}}function x(e,a,s=t){if(!s||!e)return;const n=s.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES (?, ?)");n.bind([e,typeof a=="string"?a:JSON.stringify(a)]).step(),n.finalize()}function fe(e,a=t){if(!a||!e)return;const s=a.prepare("DELETE FROM meta WHERE key = ?");s.bind([e]).step(),s.finalize()}function U(e,a=t){return!a||!e?null:a.selectValue("SELECT value FROM meta WHERE key = ?",[e])??null}function be(e){if(!N)throw new Error("SQLite not initialized");const a=new N.oo1.DB(":memory:"),s=N.wasm.scopedAllocPush();try{const n=N.wasm.allocFromTypedArray(e),i=N.wasm.allocCString("main"),r=(N.capi.SQLITE_DESERIALIZE_FREEONCLOSE||0)|(N.capi.SQLITE_DESERIALIZE_RESIZEABLE||0),l=N.capi.sqlite3_deserialize(a.pointer,i,n,e.byteLength,e.byteLength,r);if(l!==N.capi.SQLITE_OK)throw a.close(),new Error(`sqlite3_deserialize failed: ${N.capi.sqlite3_js_rc_str(l)||l}`)}finally{N.wasm.scopedAllocPop(s)}return a}self.onmessage=async function(e){const{id:a,action:s,payload:n}=e.data;try{let i;switch(s){case"init":i=await ht(n);break;case"importSnapshot":i=await gt(n);break;case"exportSnapshot":i=await Tt();break;case"exportMobileUnshared":i=await ft();break;case"markMobileShared":i=await bt();break;case"deleteSharedHistory":i=await St();break;case"markFotosShared":i=await Vt();break;case"clearSharedFotos":i=await $t();break;case"upsertMedium":i=await Nt(n);break;case"deleteMedium":i=await Lt(n);break;case"listMediums":i=await At();break;case"listMediumsPaged":i=await It(n);break;case"listHistory":i=await kt(n);break;case"listHistoryPaged":i=await Ne(n);break;case"streamHistoryChunk":i=await Ct(n);break;case"exportHistoryRange":i=await Mt(n);break;case"getHistoryEntry":i=await Zt(n);break;case"appendHistoryEntry":i=await en(n);break;case"deleteHistoryEntry":i=await tn(n);break;case"deleteHistoryRange":i=await yt(n);break;case"vacuumDatabase":i=await Dt();break;case"setArchiveLogs":i=await zt(n);break;case"listArchiveLogs":i=await vt(n);break;case"insertArchiveLog":i=await xt(n);break;case"appendImportLog":i=await Xt(n);break;case"listImportLog":i=await wt(n);break;case"appendFoto":i=await Ft(n);break;case"listFotos":i=await Ut(n);break;case"getFotoData":i=await jt(n);break;case"exportFotos":i=await Kt();break;case"deleteFoto":i=await Yt(n);break;case"updateFoto":i=await Wt(n);break;case"setFotoThumb":i=await qt(n);break;case"getFotoCounts":i=await Pt();break;case"deleteFotosByIds":i=await Bt(n);break;case"bulkUpdateFotoKategorie":i=await Ht(n);break;case"exportFotosByIds":i=await Gt(n);break;case"clearFotos":i=await Jt();break;case"deleteArchiveLog":i=await Qt(n);break;case"exportDB":i=await nn();break;case"importDB":i=await an(n);break;case"importLookupEppo":i=await sn(n);break;case"importLookupBbch":i=await rn(n);break;case"searchEppoCodes":i=await ln(n);break;case"searchBbchStages":i=await cn(n);break;case"listLookupLanguages":i=on();break;case"getLookupStats":i=await dn();break;case"listGpsPoints":i=await Ce();break;case"listGpsPointsPaged":i=await Me(n);break;case"upsertGpsPoint":i=await ye(n);break;case"deleteGpsPoint":i=await De(n);break;case"setActiveGpsPointId":i=await ze(n);break;case"getActiveGpsPointId":i=await ve();break;case"listKulturen":i=await xe();break;case"listKulturMittel":i=await Xe(n);break;case"upsertKulturMittel":i=await we(n);break;case"deleteKulturMittel":i=await Fe(n);break;case"seedInitialData":i=await Ue(n);break;case"listLagerBewegungen":i=await tt(n);break;case"upsertLagerBewegung":i=await nt(n);break;case"deleteLagerBewegung":i=await at(n);break;case"getLagerUebersicht":i=await it();break;case"listMittelStammdaten":i=await qe();break;case"listAckerflaechen":i=await Pe();break;case"upsertAckerflaeche":i=await Be(n);break;case"deleteAckerflaeche":i=await He(n);break;case"listAnbau":i=await We(n);break;case"upsertAnbau":i=await je(n);break;case"deleteAnbau":i=await Ke(n);break;case"listKulturStamm":i=await Ve();break;case"upsertKulturStamm":i=await $e(n);break;case"deleteKulturStamm":i=await Ye(n);break;case"listMassnahmen":i=await Je(n);break;case"upsertMassnahme":i=await Qe(n);break;case"deleteMassnahme":i=await Ze(n);break;case"importPsmAsMassnahmen":i=await et();break;case"listSavedEppo":i=await st(n);break;case"upsertSavedEppo":i=await rt(n);break;case"deleteSavedEppo":i=await lt(n);break;case"incrementEppoUsage":i=await ut(n);break;case"listSavedBbch":i=await ot(n);break;case"upsertSavedBbch":i=await ct(n);break;case"deleteSavedBbch":i=await dt(n);break;case"incrementBbchUsage":i=await mt(n);break;case"getFrequentEppo":i=await Et(n);break;case"getFrequentBbch":i=await _t(n);break;default:throw new Error(`Unknown action: ${s}`)}self.postMessage({id:a,ok:!0,result:i})}catch(i){self.postMessage({id:a,ok:!1,error:i.message||String(i)})}};async function ht(e={}){if(K)return{success:!0,message:"Already initialized"};try{N=await(await import(ue+"sqlite3.mjs").then(n=>n.default))({print:console.log,printErr:console.error,locateFile:n=>ue+n});const s=e.mode||pt();return t=Se(s),V=s,re(),await le(),K=!0,{success:!0,mode:s,message:`Database initialized in ${s} mode`}}catch(a){throw console.error("Failed to initialize database:",a),new Error(`Database initialization failed: ${a.message}`)}}function pt(){return typeof N?.opfs<"u"?"opfs":"memory"}function Se(e="memory"){return e==="opfs"&&N?.opfs?new N.oo1.OpfsDb("/pflanzenschutz.sqlite"):new N.oo1.DB}function re(e=t){if(!e)throw new Error("Database not initialized");e.exec(`
  ...
  `)}async function le(){if(!t)throw new Error("Database not initialized");(t.selectValue("PRAGMA user_version")||0)===0&&t.exec(`
    ...
    `),e.meta){const s={version:e.meta.version||1,company:JSON.stringify(e.meta.company||{}),defaults:JSON.stringify(e.meta.defaults||{}),fieldLabels:JSON.stringify(e.meta.fieldLabels||{}),measurementMethods:JSON.stringify(e.meta.measurementMethods||[]),archives:JSON.stringify(e.archives||{logs:[]})},n=t.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES (?, ?)");for(const[l,u]of Object.entries(s))n.bind([l,typeof u=="string"?u:JSON.stringify(u)]).step(),n.reset();if(n.finalize(),e.meta.measurementMethods&&Array.isArray(e.meta.measurementMethods)){const l=t.prepare("INSERT OR REPLACE INTO measurement_methods (id, label, type, unit, requires, config) VALUES (?, ?, ?, ?, ?, ?)");for(const u of e.meta.measurementMethods)l.bind([u.id,u.label,u.type,u.unit,JSON.stringify(u.requires||[]),JSON.stringify(u.config||{})]).step(),l.reset();l.finalize()}const i=Array.isArray(e.archives?.logs)?e.archives.logs:[],r=te(i,t);x("archives",JSON.stringify({logs:r}),t)}if(e.mediums&&Array.isArray(e.mediums)){$(t);const s=t.prepare("INSERT OR REPLACE INTO mediums (id, name, unit, method_id, value, zulassungsnummer, wartezeit, wirkstoff) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");for(const n of e.mediums){const i=n.zulassungsnummer??n.approvalNumber??n.zulassung??null;s.bind([n.id,n.name,n.unit,n.methodId||n.method_id,n.value,i,n.wartezeit??null,n.wirkstoff??null]).step(),s.reset()}s.finalize()}if(e.mediumProfiles&&Array.isArray(e.mediumProfiles)){oe(),t.exec("DELETE FROM medium_profile_mediums"),t.exec("DELETE FROM medium_profiles");const s=t.prepare("INSERT INTO medium_profiles (id, name, created_at, updated_at) VALUES (?, ?, ?, ?)"),n=t.prepare("INSERT INTO medium_profile_mediums (profile_id, medium_id, sort_order) VALUES (?, ?, ?)");for(const i of e.mediumProfiles){const r=String(i.id||"").trim();if(!r)continue;const l=String(i.name||"Profil ohne Namen"),u=i.createdAt||i.created_at||new Date().toISOString(),o=i.updatedAt||i.updated_at||u;s.bind([r,l,u,o]).step(),s.reset(),(Array.isArray(i.mediumIds)?i.mediumIds:[]).forEach((g,_)=>{g&&(n.bind([r,String(g),_]).step(),n.reset())})}s.finalize(),n.finalize()}if(e.history&&Array.isArray(e.history)){const s=t.prepare(`
      ...
      `);for(const i of e.history){const r=i.header?{...i.header}:{...i};delete r.items;const l=i.savedAt||r.savedAt||r.createdAt||new Date().toISOString();r.createdAt||(r.createdAt=l);const u=r.gpsCoordinates||{};s.bind([l,r.clientUuid||null,JSON.stringify(r),r.ersteller||null,r.standort||null,r.kultur||null,r.eppoCode||null,r.bbch||null,r.datum||null,r.dateIso||null,r.uhrzeit||null,r.usageType||null,r.areaHa??null,r.areaAr??null,r.areaSqm??null,r.waterVolume??null,r.invekos||null,r.gps||null,u.latitude??null,u.longitude??null,r.gpsPointId||null,r.qsMaschine||null,r.qsSchaderreger||null,r.qsVerantwortlicher||null,r.qsWetter||null,r.qsBehandlungsart||null]).step();const o=(t.selectValue("SELECT changes()")||0)>0,c=o?t.selectValue("SELECT last_insert_rowid()"):null;if(s.reset(),!o)continue;const g=i.items&&Array.isArray(i.items)?i.items:[];for(const _ of g){const p=_.inputs||{};n.bind([c,_.mediumId||_.medium_id||_.id||"",JSON.stringify(_),_.name||null,_.unit||null,_.methodId||null,_.methodLabel||null,_.value??null,_.total??null,_.zulassungsnummer||null,_.wartezeit??null,_.wirkstoff||null,p.areaHa??null,p.areaAr??null,p.areaSqm??null,p.waterVolume??null]).step(),n.reset()}}s.finalize(),n.finalize()}return t.exec("COMMIT"),{success:!0,message:"Snapshot imported successfully"}}catch(a){throw t.exec("ROLLBACK"),a}}async function Tt(){if(!t)throw new Error("Database not initialized");const e={meta:{version:1,company:{},defaults:{},fieldLabels:{},measurementMethods:[]},mediums:[],mediumProfiles:[],history:[],archives:{logs:[]}};t.exec({sql:"SELECT key, value FROM meta",callback:i=>{const r=i[0],l=i[1];if(!(r&&r.startsWith("lookup:")))try{const u=JSON.parse(l);r==="company"?e.meta.company=u:r==="defaults"?e.meta.defaults=u:r==="fieldLabels"?e.meta.fieldLabels=u:r==="version"?e.meta.version=u:r==="archives"&&(e.archives=u)}catch{}}}),t.exec({sql:"SELECT id, label, type, unit, requires, config FROM measurement_methods",callback:i=>{e.meta.measurementMethods.push({id:i[0],label:i[1],type:i[2],unit:i[3],requires:JSON.parse(i[4]||"[]"),config:JSON.parse(i[5]||"{}")})}}),t.exec({sql:"SELECT id, name, unit, method_id, value, zulassungsnummer, wartezeit, wirkstoff FROM mediums",callback:i=>{e.mediums.push({id:i[0],name:i[1],unit:i[2],methodId:i[3],value:i[4],zulassungsnummer:i[5]||null,wartezeit:i[6]||null,wirkstoff:i[7]||null})}});const a=new Map;t.exec({sql:`SELECT id, name, created_at, updated_at
          FROM medium_profiles
          ORDER BY name COLLATE NOCASE`,rowMode:"object",callback:i=>{a.set(i.id,{id:String(i.id),name:String(i.name??""),createdAt:i.created_at||new Date().toISOString(),updatedAt:i.updated_at||new Date().toISOString(),mediumIds:[]})}}),a.size&&t.exec({sql:`SELECT profile_id, medium_id, sort_order
            FROM medium_profile_mediums
            ORDER BY profile_id, sort_order, rowid`,rowMode:"object",callback:i=>{const r=a.get(i.profile_id);r&&r.mediumIds.push(String(i.medium_id))}}),e.mediumProfiles=Array.from(a.values());const s=ne();s.length&&(e.archives={logs:s.map(i=>Y(i))});const n=new Map;return t.exec({sql:`SELECT id, created_at, header_json, client_uuid,
            ersteller, standort, kultur, eppo_code, bbch, datum, date_iso,
            uhrzeit, usage_type, area_ha, area_ar, area_sqm, water_volume,
            invekos, gps, gps_latitude, gps_longitude, gps_point_id,
            qs_maschine, qs_schaderreger, qs_verantwortlicher, qs_wetter, qs_behandlungsart
          FROM history ORDER BY created_at DESC`,rowMode:"object",callback:i=>{let r={};try{r=JSON.parse(i.header_json||"{}")}catch(l){console.warn("Could not parse header_json",l)}n.set(i.id,{header:{createdAt:i.created_at,ersteller:i.ersteller??r.ersteller,standort:i.standort??r.standort,kultur:i.kultur??r.kultur,eppoCode:i.eppo_code??r.eppoCode,bbch:i.bbch??r.bbch,datum:i.datum??r.datum,dateIso:i.date_iso??r.dateIso,uhrzeit:i.uhrzeit??r.uhrzeit,usageType:i.usage_type??r.usageType,areaHa:i.area_ha??r.areaHa,areaAr:i.area_ar??r.areaAr,areaSqm:i.area_sqm??r.areaSqm,waterVolume:i.water_volume??r.waterVolume,invekos:i.invekos??r.invekos,gps:i.gps??r.gps,gpsCoordinates:i.gps_latitude!=null&&i.gps_longitude!=null?{latitude:i.gps_latitude,longitude:i.gps_longitude}:r.gpsCoordinates||null,gpsPointId:i.gps_point_id??r.gpsPointId,qsMaschine:i.qs_maschine??r.qsMaschine,qsSchaderreger:i.qs_schaderreger??r.qsSchaderreger,qsVerantwortlicher:i.qs_verantwortlicher??r.qsVerantwortlicher,qsWetter:i.qs_wetter??r.qsWetter,qsBehandlungsart:i.qs_behandlungsart??r.qsBehandlungsart,savedAt:r.savedAt||i.created_at,clientUuid:i.client_uuid??r.clientUuid??null},items:[]})}}),t.exec({sql:`SELECT history_id, medium_id, payload_json,
            medium_name, medium_unit, method_id, method_label, medium_value,
            calculated_total, zulassungsnummer, wartezeit, wirkstoff,
            input_area_ha, input_area_ar, input_area_sqm, input_water_volume
          FROM history_items`,rowMode:"object",callback:i=>{const r=i.history_id;if(n.has(r)){let l={};try{l=JSON.parse(i.payload_json||"{}")}catch(u){console.warn("Could not parse payload_json",u)}n.get(r).items.push({id:i.medium_id??l.id,mediumId:i.medium_id??l.mediumId,name:i.medium_name??l.name,unit:i.medium_unit??l.unit,methodId:i.method_id??l.methodId,methodLabel:i.method_label??l.methodLabel,value:i.medium_value??l.value,total:i.calculated_total??l.total,zulassungsnummer:i.zulassungsnummer??l.zulassungsnummer,wartezeit:i.wartezeit??l.wartezeit,wirkstoff:i.wirkstoff??l.wirkstoff,inputs:{areaHa:i.input_area_ha??l.inputs?.areaHa,areaAr:i.input_area_ar??l.inputs?.areaAr,areaSqm:i.input_area_sqm??l.inputs?.areaSqm,waterVolume:i.input_water_volume??l.inputs?.waterVolume}})}}}),e.history=Array.from(n.values()).map(i=>({...i.header,items:i.items})),e}async function ft(){if(!t)throw new Error("Database not initialized");const e={meta:{version:1,company:{},defaults:{},fieldLabels:{},measurementMethods:[]},mediums:[],mediumProfiles:[],history:[],archives:{logs:[]}};t.exec({sql:"SELECT key, value FROM meta WHERE key IN ('company', 'version')",callback:s=>{try{const n=JSON.parse(s[1]);s[0]==="company"?e.meta.company=n:s[0]==="version"&&(e.meta.version=n)}catch{}}});const a=new Map;if(t.exec({sql:`SELECT id, created_at, header_json, client_uuid,
            ersteller, standort, kultur, eppo_code, bbch, datum, date_iso,
            uhrzeit, usage_type, area_ha, area_ar, area_sqm, water_volume,
            invekos, gps, gps_latitude, gps_longitude, gps_point_id,
            qs_maschine, qs_schaderreger, qs_verantwortlicher, qs_wetter, qs_behandlungsart
          FROM history WHERE mobile_shared_at IS NULL ORDER BY created_at DESC`,rowMode:"object",callback:s=>{let n={};try{n=JSON.parse(s.header_json||"{}")}catch{}a.set(s.id,{header:{createdAt:s.created_at,ersteller:s.ersteller??n.ersteller,standort:s.standort??n.standort,kultur:s.kultur??n.kultur,eppoCode:s.eppo_code??n.eppoCode,bbch:s.bbch??n.bbch,datum:s.datum??n.datum,dateIso:s.date_iso??n.dateIso,uhrzeit:s.uhrzeit??n.uhrzeit,usageType:s.usage_type??n.usageType,areaHa:s.area_ha??n.areaHa,areaAr:s.area_ar??n.areaAr,areaSqm:s.area_sqm??n.areaSqm,waterVolume:s.water_volume??n.waterVolume,invekos:s.invekos??n.invekos,gps:s.gps??n.gps,gpsCoordinates:s.gps_latitude!=null&&s.gps_longitude!=null?{latitude:s.gps_latitude,longitude:s.gps_longitude}:n.gpsCoordinates||null,gpsPointId:s.gps_point_id??n.gpsPointId,qsMaschine:s.qs_maschine??n.qsMaschine,qsSchaderreger:s.qs_schaderreger??n.qsSchaderreger,qsVerantwortlicher:s.qs_verantwortlicher??n.qsVerantwortlicher,qsWetter:s.qs_wetter??n.qsWetter,qsBehandlungsart:s.qs_behandlungsart??n.qsBehandlungsart,savedAt:n.savedAt||s.created_at,clientUuid:s.client_uuid??n.clientUuid??null},items:[]})}}),t.exec({sql:`SELECT history_id, medium_id, payload_json,
            medium_name, medium_unit, method_id, method_label, medium_value,
            calculated_total, zulassungsnummer, wartezeit, wirkstoff,
            input_area_ha, input_area_ar, input_area_sqm, input_water_volume
          FROM history_items`,rowMode:"object",callback:s=>{if(a.has(s.history_id)){let n={};try{n=JSON.parse(s.payload_json||"{}")}catch{}a.get(s.history_id).items.push({id:s.medium_id??n.id,mediumId:s.medium_id??n.mediumId,name:s.medium_name??n.name,unit:s.medium_unit??n.unit,methodId:s.method_id??n.methodId,methodLabel:s.method_label??n.methodLabel,value:s.medium_value??n.value,total:s.calculated_total??n.total,zulassungsnummer:s.zulassungsnummer??n.zulassungsnummer,wartezeit:s.wartezeit??n.wartezeit,wirkstoff:s.wirkstoff??n.wirkstoff,inputs:{areaHa:s.input_area_ha??n.inputs?.areaHa,areaAr:s.input_area_ar??n.inputs?.areaAr,areaSqm:s.input_area_sqm??n.inputs?.areaSqm,waterVolume:s.input_water_volume??n.inputs?.waterVolume}})}}}),e.history=Array.from(a.values()).map(s=>({...s.header,items:s.items})),a.size>0){t.exec({sql:"SELECT key, value FROM meta",callback:i=>{const r=i[0],l=i[1];if(!(r&&r.startsWith("lookup:")))try{const u=JSON.parse(l);r==="defaults"?e.meta.defaults=u:r==="fieldLabels"?e.meta.fieldLabels=u:r==="archives"&&(e.archives=u)}catch{}}}),t.exec({sql:"SELECT id, label, type, unit, requires, config FROM measurement_methods",callback:i=>{e.meta.measurementMethods.push({id:i[0],label:i[1],type:i[2],unit:i[3],requires:JSON.parse(i[4]||"[]"),config:JSON.parse(i[5]||"{}")})}}),t.exec({sql:"SELECT id, name, unit, method_id, value, zulassungsnummer, wartezeit, wirkstoff FROM mediums",callback:i=>{e.mediums.push({id:i[0],name:i[1],unit:i[2],methodId:i[3],value:i[4],zulassungsnummer:i[5]||null,wartezeit:i[6]||null,wirkstoff:i[7]||null})}});const s=new Map;t.exec({sql:"SELECT id, name, created_at, updated_at FROM medium_profiles ORDER BY name COLLATE NOCASE",rowMode:"object",callback:i=>{s.set(i.id,{id:String(i.id),name:String(i.name??""),createdAt:i.created_at||new Date().toISOString(),updatedAt:i.updated_at||new Date().toISOString(),mediumIds:[]})}}),s.size&&t.exec({sql:"SELECT profile_id, medium_id, sort_order FROM medium_profile_mediums ORDER BY profile_id, sort_order, rowid",rowMode:"object",callback:i=>{const r=s.get(i.profile_id);r&&r.mediumIds.push(String(i.medium_id))}}),e.mediumProfiles=Array.from(s.values());const n=ne();n.length&&(e.archives={logs:n.map(i=>Y(i))})}return e}async function bt(){if(!t)throw new Error("Database not initialized");const e=t.selectValue("SELECT COUNT(*) FROM history WHERE mobile_shared_at IS NULL")||0;return e>0&&t.exec("UPDATE history SET mobile_shared_at = datetime('now') WHERE mobile_shared_at IS NULL"),{marked:e}}async function St(){if(!t)throw new Error("Database not initialized");const e=t.selectValue("SELECT COUNT(*) FROM history WHERE mobile_shared_at IS NOT NULL")||0;return e>0&&(t.exec("DELETE FROM history_items WHERE history_id IN (SELECT id FROM history WHERE mobile_shared_at IS NOT NULL)"),t.exec("DELETE FROM history WHERE mobile_shared_at IS NOT NULL")),{deleted:e}}async function Nt(e){if(!t)throw new Error("Database not initialized");$();const a=t.prepare("INSERT OR REPLACE INTO mediums (id, name, unit, method_id, value, zulassungsnummer, wartezeit, wirkstoff) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");return a.bind([e.id,e.name,e.unit,e.methodId||e.method_id,e.value,e.zulassungsnummer??e.approvalNumber??e.zulassung??null,e.wartezeit??null,e.wirkstoff??null]).step(),a.finalize(),{success:!0,id:e.id}}async function Lt(e){if(!t)throw new Error("Database not initialized");const a=t.prepare("DELETE FROM mediums WHERE id = ?");return a.bind([e]).step(),a.finalize(),{success:!0}}async function At(){if(!t)throw new Error("Database not initialized");$();const e=[];return t.exec({sql:"SELECT id, name, unit, method_id, value, zulassungsnummer, wartezeit, wirkstoff FROM mediums",callback:a=>{e.push({id:a[0],name:a[1],unit:a[2],methodId:a[3],value:a[4],zulassungsnummer:a[5]||null,wartezeit:a[6]||null,wirkstoff:a[7]||null})}}),e}function Rt(e){return e?{id:e.id!=null?String(e.id):"",name:e.name!=null?String(e.name):"",unit:e.unit!=null?String(e.unit):"",methodId:e.method_id!=null?String(e.method_id):e.methodId!=null?String(e.methodId):null,value:Number(e.value??0),zulassungsnummer:e.zulassungsnummer!=null?String(e.zulassungsnummer):e.zulassung!=null?String(e.zulassung):null,wartezeit:e.wartezeit!=null?Number(e.wartezeit):null,wirkstoff:e.wirkstoff!=null?String(e.wirkstoff):null}:null}async function It(e={}){if(!t)throw new Error("Database not initialized");const{cursor:a=null,limit:s,pageSize:n,search:i,includeTotal:r=!1}=e||{},l=H(n??s,50,500),u=typeof i=="string"?i.trim():"",o=[],c=[];if(u){o.push("(LOWER(name) LIKE LOWER(?) OR LOWER(COALESCE(zulassungsnummer, '')) LIKE LOWER(?))");const m=`%${u}%`;c.push(m,m)}const g=a&&Number.isFinite(Number(a.rowid))?Number(a.rowid):null,_=[...o],p=[...c];g!==null&&(_.push("rowid > ?"),p.push(g));const b=_.length?`WHERE ${_.join(" AND ")}`:"",f=t.prepare(`SELECT rowid AS cursor_rowid, id, name, unit, method_id, value, zulassungsnummer
     FROM mediums
     ${b}
     ORDER BY rowid ASC
    ...
    END,
    created_at
  )`,s=[],n=[];if(e.startDate&&(s.push(`datetime(${a}) >= datetime(?)`),n.push(e.startDate)),e.endDate&&(s.push(`datetime(${a}) <= datetime(?)`),n.push(e.endDate)),[["creator","$.ersteller"],["location","$.standort"],["crop","$.kultur"],["usageType","$.usageType"],["eppoCode","$.eppoCode"],["invekos","$.invekos"],["bbch","$.bbch"]].forEach(([r,l])=>{const u=e?.[r];typeof u=="string"&&u.trim()&&(s.push(`LOWER(json_extract(header_json, '${l}')) LIKE LOWER(?)`),n.push(`%${u.trim()}%`))}),typeof e?.text=="string"&&e.text.trim()){const r=`%${e.text.trim()}%`,l=["$.ersteller","$.standort","$.kultur","$.usageType","$.eppoCode","$.invekos","$.bbch","$.gps"],u=l.map(o=>`LOWER(json_extract(header_json, '${o}')) LIKE LOWER(?)`);s.push(`(${u.join(" OR ")})`);for(let o=0;o<l.length;o+=1)n.push(r)}return{historyDateExpr:a,whereSql:s.length?`WHERE ${s.join(" AND ")}`:"",whereParams:n}}function Ot(e,a,s){if(!e||!e.createdAt)return null;const n=s==="ASC"?">":"<",i=String(e.createdAt),r=Number(e.id??e.historyId??e.rowid??0);return{clause:`(
      datetime(${a}) ${n} datetime(?)
      OR (datetime(${a}) = datetime(?) AND history.id ${n} ?)
    )`,params:[i,i,r]}}async function kt({page:e=1,pageSize:a=50,filters:s={},sortDirection:n="desc"}={}){if(!t)throw new Error("Database not initialized");const i=String(n).toLowerCase()==="asc"?"ASC":"DESC",r=H(a,50,500),l=Number.isFinite(Number(e))?Math.max(1,Number(e)):1,u=(l-1)*r,{historyDateExpr:o,whereSql:c,whereParams:g}=ee(s),_=[],p={sql:`
      SELECT id, created_at, header_json
      FROM history
      ${c}
      ORDER BY datetime(${o}) ${i}, rowid ${i}
      LIMIT ${r} OFFSET ${u}
    `,callback:f=>{const O=JSON.parse(f[2]||"{}");_.push({id:f[0],...O})}};g.length&&(p.bind=[...g]),t.exec(p);const b=t.selectValue(`SELECT COUNT(*) FROM history ${c}`,g.length?[...g]:void 0);return{items:_,page:l,pageSize:r,totalCount:Number(b)||0,totalPages:Math.ceil((Number(b)||0)/r)}}async function Ne({cursor:e=null,pageSize:a=50,filters:s={},sortDirection:n="desc",includeItems:i=!1,includeTotal:r=!1}={}){if(!t)throw new Error("Database not initialized");const l=String(n).toLowerCase()==="asc"?"ASC":"DESC",u=H(a,50,500),o=u+1,{historyDateExpr:c,whereSql:g,whereParams:_}=ee(s),p=Ot(e,c,l),b=[],f=[];g?.trim()&&(b.push(g.replace(/^\s*WHERE\s+/i,"")),_.length&&f.push(..._)),p&&(b.push(p.clause),f.push(...p.params));const O=b.length>0?`WHERE ${b.join(" AND ")}`:"",A=[];t.exec({sql:`
      SELECT
        history.id AS id,
  ...
  `,_=u.length?[...u,n]:[n];return t.exec({sql:g,bind:_,rowMode:"object",callback:p=>{try{const b=JSON.parse(p.header_json||"{}"),f=JSON.parse(p.items_json||"[]"),O=Array.isArray(f)?f.map(A=>{if(A==null)return null;if(typeof A=="string")try{return JSON.parse(A)}catch(k){return console.warn("Konnte History-Item nicht parsen",k),null}return A}).filter(A=>A!==null):[];c.push(p.history_id),o.push({...b,items:O})}catch(b){console.warn("Archiv-Export konnte nicht gelesen werden",b)}}}),{entries:o,historyIds:c}}async function yt({filters:e={}}={}){if(!t)throw new Error("Database not initialized");const{whereSql:a,whereParams:s}=ee(e);t.exec("BEGIN TRANSACTION");try{const n={sql:`DELETE FROM history ${a}`};return s.length&&(n.bind=[...s]),t.exec(n),t.exec("COMMIT"),{success:!0}}catch(n){throw t.exec("ROLLBACK"),n}}async function Dt(){if(!t)throw new Error("Database not initialized");return t.exec("VACUUM"),{success:!0}}async function zt(e={}){if(!t)throw new Error("Database not initialized");const a=Array.isArray(e.logs)?e.logs:[];t.exec("BEGIN TRANSACTION");try{const s=te(a,t);return x("archives",JSON.stringify({logs:s}),t),t.exec("COMMIT"),{success:!0,count:s.length}}catch(s){throw t.exec("ROLLBACK"),s}}async function vt({limit:e=50,cursor:a=null,sortDirection:s="desc"}={}){if(!t)throw new Error("Database not initialized");q();const n=String(s).toLowerCase()==="asc"?"ASC":"DESC",i=Math.min(Math.max(Number(e)||1,1),500),r=i+1,l=(()=>{if(!a||!a.archivedAt)return null;const f=n==="ASC"?">":"<",O=f,A=String(a.archivedAt),k=a.id?String(a.id):"";return{clause:`(
        ...
        OR (datetime(archived_at) = datetime(?) AND id ${O} ?)
          ...
          LIMIT ?`,bind:[...o,r],rowMode:"object",callback:f=>g.push(f)});let _=null;const p=g.length>i;if(p){const f=g.pop();f&&(_={id:String(f.id),archivedAt:f.archived_at})}return{items:g.map(f=>ae(f)).filter(f=>!!f),nextCursor:_,hasMore:p,pageSize:i,sortDirection:n}}async function xt(e={}){if(!t)throw new Error("Database not initialized");q();const a=ce(e);if(!a)throw new Error("Ungültiger Archiv-Eintrag");const s=t.prepare(`INSERT OR REPLACE INTO archive_logs
      (id, archived_at, start_date, end_date, entry_count, file_name, storage_hint, note, metadata_json)
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);return s.bind([a,e.source!=null?String(e.source):null,e.device!=null?String(e.device):null,Number(e.added)||0,Number(e.skipped)||0,e.rangeStart!=null?String(e.rangeStart):null,e.rangeEnd!=null?String(e.rangeEnd):null,e.note!=null?String(e.note):null]).step(),s.finalize(),{id:t.selectValue("SELECT last_insert_rowid()"),importedAt:a}}async function wt(e={}){if(!t)throw new Error("Database not initialized");se();const a=Math.min(Math.max(Number(e?.limit)||50,1),500),s=[];return t.exec({sql:`SELECT id, imported_at, source, device, added, skipped, range_start, range_end, note
          FROM import_log
          ORDER BY datetime(imported_at) DESC, id DESC
          LIMIT ?`,bind:[a],rowMode:"object",callback:n=>{s.push({id:n.id,importedAt:n.imported_at,source:n.source,device:n.device,added:Number(n.added)||0,skipped:Number(n.skipped)||0,rangeStart:n.range_start,rangeEnd:n.range_end,note:n.note})}}),{items:s}}async function Ft(e={}){if(!t)throw new Error("Database not initialized");C();const a=e.clientUuid?String(e.clientUuid):null;if(a)try{const r=t.selectValue("SELECT id FROM fotos WHERE client_uuid = ? LIMIT 1",[a]);if(r!=null)return{id:r,duplicate:!0}}catch{}if(!e.data)throw new Error("Foto-Daten fehlen");const s=e.createdAt||new Date().toISOString(),n=t.prepare(`INSERT INTO fotos
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);try{n.bind([a,s,e.entryUuid!=null?String(e.entryUuid):null,e.kategorie!=null?String(e.kategorie):null,e.titel!=null?String(e.titel):null,e.standort!=null?String(e.standort):null,e.kultur!=null?String(e.kultur):null,e.gpsLatitude??null,e.gpsLongitude??null,e.notiz!=null?String(e.notiz):null,e.device!=null?String(e.device):null,e.mime!=null?String(e.mime):"image/jpeg",e.width??null,e.height??null,e.bytes??null,String(e.data),e.thumb!=null?String(e.thumb):null]).step(),n.finalize()}catch(r){try{n.finalize()}catch{}if(a&&/UNIQUE|constraint/i.test(String(r&&r.message))){const l=t.selectValue("SELECT id FROM fotos WHERE client_uuid = ? LIMIT 1",[a]);if(l!=null)return{id:l,duplicate:!0}}throw r}return{id:t.selectValue("SELECT last_insert_rowid()"),duplicate:!1}}async function Ut(e={}){if(!t)throw new Error("Database not initialized");C();const a=Math.min(Math.max(Number(e?.limit)||200,1),1e5),s=[];return t.exec({sql:`SELECT id, client_uuid, created_at, entry_uuid, kategorie, titel,
                 standort, kultur, gps_latitude, gps_longitude, notiz, device, mime,
                 width, height, bytes, data_thumb
          FROM fotos ORDER BY created_at DESC, id DESC LIMIT ?`,bind:[a],rowMode:"object",callback:n=>{s.push({id:n.id,clientUuid:n.client_uuid,createdAt:n.created_at,entryUuid:n.entry_uuid,kategorie:n.kategorie,titel:n.titel,standort:n.standort,kultur:n.kultur,gpsLatitude:n.gps_latitude,gpsLongitude:n.gps_longitude,notiz:n.notiz,device:n.device,mime:n.mime,width:n.width,height:n.height,bytes:n.bytes,thumb:n.data_thumb||null})}}),{items:s}}async function qt(e={}){if(!t)throw new Error("Database not initialized");C();const a=e&&e.id!=null?Number(e.id):null;if(a==null||!e.thumb)return{success:!1};const s=t.prepare("UPDATE fotos SET data_thumb = ? WHERE id = ?");return s.bind([String(e.thumb),a]).step(),s.finalize(),{success:!0}}async function Pt(){if(!t)throw new Error("Database not initialized");C();const e={};let a=0,s=0;return t.exec({sql:"SELECT kategorie, COUNT(*) AS n, COALESCE(SUM(bytes),0) AS b FROM fotos GROUP BY kategorie",rowMode:"object",callback:n=>{e[n.kategorie||""]=Number(n.n)||0,a+=Number(n.n)||0,s+=Number(n.b)||0}}),{counts:e,total:a,totalBytes:s}}async function Bt(e={}){if(!t)throw new Error("Database not initialized");C();const a=Array.isArray(e?.ids)?e.ids.map(n=>Number(n)).filter(n=>Number.isFinite(n)):[];if(!a.length)return{success:!0,deleted:0};const s=a.map(()=>"?").join(",");return t.exec({sql:`DELETE FROM fotos WHERE id IN (${s})`,bind:a}),{success:!0,deleted:a.length}}async function Ht(e={}){if(!t)throw new Error("Database not initialized");C();const a=Array.isArray(e?.ids)?e.ids.map(i=>Number(i)).filter(i=>Number.isFinite(i)):[],s=e?.kategorie!=null?String(e.kategorie):null;if(!a.length)return{success:!0,updated:0};const n=a.map(()=>"?").join(",");return t.exec({sql:`UPDATE fotos SET kategorie = ? WHERE id IN (${n})`,bind:[s,...a]}),{success:!0,updated:a.length}}async function Gt(e={}){if(!t)throw new Error("Database not initialized");C();const a=Array.isArray(e?.ids)?e.ids.map(i=>Number(i)).filter(i=>Number.isFinite(i)):[];if(!a.length)return{items:[]};const s=a.map(()=>"?").join(","),n=[];return t.exec({sql:`SELECT client_uuid, created_at, entry_uuid, kategorie, titel, standort,
                 kultur, gps_latitude, gps_longitude, notiz, device, mime, width, height, bytes, data
          FROM fotos WHERE id IN (${s}) ORDER BY created_at ASC, id ASC`,bind:a,rowMode:"object",callback:i=>{n.push({clientUuid:i.client_uuid,createdAt:i.created_at,entryUuid:i.entry_uuid,kategorie:i.kategorie,titel:i.titel,standort:i.standort,kultur:i.kultur,gpsLatitude:i.gps_latitude,gpsLongitude:i.gps_longitude,notiz:i.notiz,device:i.device,mime:i.mime,width:i.width,height:i.height,bytes:i.bytes,data:i.data})}}),{items:n}}async function Wt(e={}){if(!t)throw new Error("Database not initialized");C();const a=e&&e.id!=null?Number(e.id):null;if(a==null)throw new Error("Foto-ID fehlt");const s=[],n=[],i=(l,u)=>{if(Object.prototype.hasOwnProperty.call(e,l)){s.push(`${u} = ?`);const o=e[l];n.push(o==null||o===""?null:o)}};if(i("titel","titel"),i("kategorie","kategorie"),i("kultur","kultur"),i("standort","standort"),i("notiz","notiz"),i("gpsLatitude","gps_latitude"),i("gpsLongitude","gps_longitude"),!s.length)return{success:!0};n.push(a);const r=t.prepare(`UPDATE fotos SET ${s.join(", ")} WHERE id = ?`);return r.bind(n).step(),r.finalize(),{success:!0}}async function jt(e={}){if(!t)throw new Error("Database not initialized");C();const a=e&&e.id!=null?Number(e.id):null;if(a==null)return{data:null};let s=null,n="image/jpeg";return t.exec({sql:"SELECT data, mime FROM fotos WHERE id = ? LIMIT 1",bind:[a],rowMode:"object",callback:i=>{s=i.data??null,n=i.mime||"image/jpeg"}}),{data:s,mime:n}}async function Kt(){if(!t)throw new Error("Database not initialized");C();const e=[];return t.exec({sql:`SELECT client_uuid, created_at, entry_uuid, kategorie, titel, standort,
                 kultur, gps_latitude, gps_longitude, notiz, device, mime, width, height, bytes, data
          FROM fotos WHERE mobile_shared_at IS NULL ORDER BY created_at ASC, id ASC`,rowMode:"object",callback:a=>{e.push({clientUuid:a.client_uuid,createdAt:a.created_at,entryUuid:a.entry_uuid,kategorie:a.kategorie,titel:a.titel,standort:a.standort,kultur:a.kultur,gpsLatitude:a.gps_latitude,gpsLongitude:a.gps_longitude,notiz:a.notiz,device:a.device,mime:a.mime,width:a.width,height:a.height,bytes:a.bytes,data:a.data})}}),{items:e}}async function Vt(){if(!t)throw new Error("Database not initialized");C();const e=t.selectValue("SELECT COUNT(*) FROM fotos WHERE mobile_shared_at IS NULL")||0;return e>0&&t.exec("UPDATE fotos SET mobile_shared_at = datetime('now') WHERE mobile_shared_at IS NULL"),{marked:e}}async function $t(){if(!t)throw new Error("Database not initialized");C();const e=t.selectValue("SELECT COUNT(*) FROM fotos WHERE mobile_shared_at IS NOT NULL")||0;return e>0&&t.exec("DELETE FROM fotos WHERE mobile_shared_at IS NOT NULL"),{deleted:e}}async function Yt(e={}){if(!t)throw new Error("Database not initialized");C();const a=e&&e.id!=null?Number(e.id):null;if(a==null)throw new Error("Foto-ID fehlt");const s=t.prepare("DELETE FROM fotos WHERE id = ?");return s.bind([a]).step(),s.finalize(),{success:!0}}async function Jt(){if(!t)throw new Error("Database not initialized");C();const e=t.selectValue("SELECT COUNT(*) FROM fotos")||0;return t.exec("DELETE FROM fotos"),{success:!0,deleted:Number(e)}}async function Qt(e={}){if(!t)throw new Error("Database not initialized");q();const a=e&&e.id?String(e.id):"";if(!a)throw new Error("Archiv-Log-ID fehlt");const s=t.prepare("DELETE FROM archive_logs WHERE id = ?");return s.bind([a]).step(),s.finalize(),de(),{success:!0}}async function Zt(e){if(!t)throw new Error("Database not initialized");let a=null;if(t.exec({sql:`SELECT id, created_at, header_json,
            ersteller, standort, kultur, eppo_code, bbch, datum, date_iso,
            uhrzeit, usage_type, area_ha, area_ar, area_sqm, water_volume,
            invekos, gps, gps_latitude, gps_longitude, gps_point_id,
            qs_maschine, qs_schaderreger, qs_verantwortlicher, qs_wetter, qs_behandlungsart,
            company_name, company_address, company_headline, company_email
          FROM history WHERE id = ?`,bind:[e],rowMode:"object",callback:s=>{let n={};if(s.header_json)try{n=JSON.parse(s.header_json)}catch(i){console.warn("Could not parse header_json",i)}a={company:{name:s.company_name??n.company?.name??null,address:s.company_address??n.company?.address??null,headline:s.company_headline??n.company?.headline??null,contactEmail:s.company_email??n.company?.contactEmail??null},id:s.id,createdAt:s.created_at,ersteller:s.ersteller??n.ersteller,standort:s.standort??n.standort,kultur:s.kultur??n.kultur,eppoCode:s.eppo_code??n.eppoCode,bbch:s.bbch??n.bbch,datum:s.datum??n.datum,dateIso:s.date_iso??n.dateIso,uhrzeit:s.uhrzeit??n.uhrzeit,usageType:s.usage_type??n.usageType,areaHa:s.area_ha??n.areaHa,areaAr:s.area_ar??n.areaAr,areaSqm:s.area_sqm??n.areaSqm,waterVolume:s.water_volume??n.waterVolume,invekos:s.invekos??n.invekos,gps:s.gps??n.gps,gpsCoordinates:s.gps_latitude!=null&&s.gps_longitude!=null?{latitude:s.gps_latitude,longitude:s.gps_longitude}:n.gpsCoordinates||null,gpsPointId:s.gps_point_id??n.gpsPointId,qsMaschine:s.qs_maschine??n.qsMaschine,qsSchaderreger:s.qs_schaderreger??n.qsSchaderreger,qsVerantwortlicher:s.qs_verantwortlicher??n.qsVerantwortlicher,qsWetter:s.qs_wetter??n.qsWetter,qsBehandlungsart:s.qs_behandlungsart??n.qsBehandlungsart,savedAt:n.savedAt||s.created_at,items:[]}}}),!a)throw new Error("History entry not found");return t.exec({sql:`SELECT id, medium_id, payload_json,
            medium_name, medium_unit, method_id, method_label, medium_value,
            calculated_total, zulassungsnummer, wartezeit, wirkstoff,
            input_area_ha, input_area_ar, input_area_sqm, input_water_volume
          FROM history_items WHERE history_id = ?`,bind:[e],rowMode:"object",callback:s=>{let n={};if(s.payload_json)try{n=JSON.parse(s.payload_json)}catch(i){console.warn("Could not parse payload_json",i)}a.items.push({id:s.medium_id??n.id,mediumId:s.medium_id??n.mediumId,name:s.medium_name??n.name,unit:s.medium_unit??n.unit,methodId:s.method_id??n.methodId,methodLabel:s.method_label??n.methodLabel,value:s.medium_value??n.value,total:s.calculated_total??n.total,zulassungsnummer:s.zulassungsnummer??n.zulassungsnummer,wartezeit:s.wartezeit??n.wartezeit,wirkstoff:s.wirkstoff??n.wirkstoff,inputs:{areaHa:s.input_area_ha??n.inputs?.areaHa,areaAr:s.input_area_ar??n.inputs?.areaAr,areaSqm:s.input_area_sqm??n.inputs?.areaSqm,waterVolume:s.input_water_volume??n.inputs?.waterVolume}})}}),a}async function en(e){if(!t)throw new Error("Database not initialized");const a=e&&e.header?e.header:e||{},s=a&&a.clientUuid?String(a.clientUuid):null;if(s)try{const n=t.selectValue("SELECT id FROM history WHERE client_uuid = ? LIMIT 1",[s]);if(n!=null)return{id:n,duplicate:!0}}catch{}t.exec("BEGIN TRANSACTION");try{const n=e.header?{...e.header}:{...e};delete n.items;const i=e.savedAt||n.savedAt||n.createdAt||new Date().toISOString();n.createdAt||(n.createdAt=i);const r=n.gpsCoordinates||{},l=n.company||{},u=t.prepare(`
      INSERT INTO history (
        created_at, client_uuid, header_json,
        ersteller, standort, kultur, eppo_code, bbch, datum, date_iso,
        uhrzeit, usage_type, area_ha, area_ar, area_sqm, water_volume,
    ...
    `);u.bind([i,s,JSON.stringify(n),n.ersteller||null,n.standort||null,n.kultur||null,n.eppoCode||null,n.bbch||null,n.datum||null,n.dateIso||null,n.uhrzeit||null,n.usageType||null,n.areaHa??null,n.areaAr??null,n.areaSqm??null,n.waterVolume??null,n.invekos||null,n.gps||null,r.latitude??null,r.longitude??null,n.gpsPointId||null,n.qsMaschine||null,n.qsSchaderreger||null,n.qsVerantwortlicher||null,n.qsWetter||null,n.qsBehandlungsart||null,l.name||null,l.address||null,l.headline||null,l.contactEmail||null]).step();const o=t.selectValue("SELECT last_insert_rowid()");u.finalize();const c=e.items&&Array.isArray(e.items)?e.items:[];if(c.length){const g=t.prepare(`
      ...
      `);for(const _ of c){const p=_.inputs||{};g.bind([o,_.mediumId||_.medium_id||_.id||"",JSON.stringify(_),_.name||null,_.unit||null,_.methodId||null,_.methodLabel||null,_.value??null,_.total??null,_.zulassungsnummer||null,_.wartezeit??null,_.wirkstoff||null,p.areaHa??null,p.areaAr??null,p.areaSqm??null,p.waterVolume??null]).step(),g.reset()}g.finalize()}return t.exec("COMMIT"),{success:!0,id:o}}catch(n){if(t.exec("ROLLBACK"),s&&/UNIQUE|constraint/i.test(String(n&&n.message)))try{const i=t.selectValue("SELECT id FROM history WHERE client_uuid = ? LIMIT 1",[s]);if(i!=null)return{id:i,duplicate:!0}}catch{}throw n}}async function tn(e){if(!t)throw new Error("Database not initialized");const a=t.prepare("DELETE FROM history WHERE id = ?");return a.bind([e]).step(),a.finalize(),{success:!0}}async function nn(){if(!t)throw new Error("Database not initialized");const e=N.capi.sqlite3_js_db_export(t.pointer);return{data:e.buffer.slice(e.byteOffset,e.byteOffset+e.byteLength)}}async function an(e){if(!t)throw new Error("Database not initialized");const a=e instanceof Uint8Array?e:new Uint8Array(e);if(V==="opfs"&&N?.oo1?.OpfsDb&&N?.opfs)return t.close(),await N.oo1.OpfsDb.importDb("/pflanzenschutz.sqlite",a),t=Se("opfs"),re(),await le(),V="opfs",K=!0,{success:!0,mode:"opfs"};t.close();const s=new N.oo1.DB,n=N.wasm.scopedAllocPush();try{const i=N.wasm.allocFromTypedArray(a),r=N.wasm.allocCString("main"),l=(N.capi.SQLITE_DESERIALIZE_FREEONCLOSE||0)|(N.capi.SQLITE_DESERIALIZE_RESIZEABLE||0),u=N.capi.sqlite3_deserialize(s.pointer,r,i,a.byteLength,a.byteLength,l);if(u!==N.capi.SQLITE_OK)throw s.close(),new Error(`sqlite3_deserialize failed: ${N.capi.sqlite3_js_rc_str(u)||u}`)}finally{N.wasm.scopedAllocPop(n)}return t=s,re(),await le(),V="memory",K=!0,{success:!0,mode:"memory"}}async function sn(e={}){if(!t)throw new Error("Database not initialized");if(!N)throw new Error("SQLite not initialized");const{data:a}=e;if(!a)throw new Error("Keine Daten zum Importieren übergeben");const s=a instanceof Uint8Array?a:new Uint8Array(a);console.log(`[EPPO Import] Lade Datenbank, ${s.length} bytes`);const n=be(s),i=[];let r=0;try{const l=[];n.exec({sql:"SELECT name FROM sqlite_master WHERE type='table'",callback:u=>{u&&u[0]&&l.push(u[0])}}),console.log("[EPPO Import] Gefundene Tabellen:",l),l.includes("eppo")?(console.log('[EPPO Import] Nutze neue deutsche Tabelle "eppo"'),n.exec({sql:"SELECT code, name FROM eppo ORDER BY name",callback:u=>{if(!u||!u[0])return;const o=String(u[0]).trim().toUpperCase(),c=u[1]?String(u[1]).trim():o;i.push({code:o,name:c,language:"DE",dtcode:null,dtLabel:"plant",languageLabel:"Deutsch",authority:null,nameDe:c,nameEn:null,nameLa:null})}})):l.includes("t_codes")?(console.log("[EPPO Import] Nutze alte Struktur mit t_codes"),n.exec({sql:`
        ...
        `,callback:u=>{if(!u||!u[0])return;const o=String(u[0]).trim().toUpperCase(),c=u[1]?String(u[1]).trim():o;i.push({code:o,name:c,language:"DE",dtcode:null,dtLabel:"plant",languageLabel:"Deutsch",authority:null,nameDe:c,nameEn:null,nameLa:null})}})):console.error("[EPPO Import] Keine bekannte Tabellenstruktur gefunden!"),r=i.length,console.log(`[EPPO Import] ${i.length} Einträge gelesen`)}finally{n.close()}t.exec("BEGIN TRANSACTION");try{if(w(t),t.exec("DELETE FROM lookup_eppo_codes"),i.length){const l=t.prepare(`INSERT OR REPLACE INTO lookup_eppo_codes (
          ...
          name_en,
          name_la
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);for(const u of i)l.bind([u.code,u.name,u.language||"",u.dtcode,1,u.dtLabel,u.languageLabel,u.authority,u.nameDe,u.nameEn,u.nameLa]).step(),l.reset();l.finalize()}return x("lookup:eppo:lastImport",new Date().toISOString()),x("lookup:eppo:count",String(r||i.length)),t.exec("COMMIT"),{success:!0,count:i.length}}catch(l){throw t.exec("ROLLBACK"),console.error("EPPO-Lookup-Import fehlgeschlagen",l),l}}async function rn(e={}){if(!t)throw new Error("Database not initialized");if(!N)throw new Error("SQLite not initialized");const{data:a}=e;if(!a)throw new Error("Keine Daten zum Importieren übergeben");const s=a instanceof Uint8Array?a:new Uint8Array(a);console.log(`[BBCH Import] Lade Datenbank, ${s.length} bytes`);const n=be(s),i=[];try{const r=[];n.exec({sql:"SELECT name FROM sqlite_master WHERE type='table'",callback:l=>{l&&l[0]&&r.push(l[0])}}),console.log("[BBCH Import] Gefundene Tabellen:",r),r.includes("bbch")?(console.log('[BBCH Import] Nutze neue deutsche Tabelle "bbch"'),n.exec({sql:"SELECT code, label, stage_group FROM bbch ORDER BY CAST(code AS INTEGER)",callback:l=>{if(!l||!l[0])return;const u=String(l[0]).trim(),o=l[1]?String(l[1]).trim():u,c=l[2]!=null?Number(l[2]):null;i.push({code:u,label:o,principal:c,secondary:null,definition:o,kind:null})}})):r.includes("bbch_stage")?(console.log("[BBCH Import] Nutze alte Struktur mit bbch_stage"),n.exec({sql:"SELECT bbch_code, label_de, definition_1, definition_2, principal_stage FROM bbch_stage ORDER BY CAST(bbch_code AS INTEGER)",callback:l=>{if(!l||!l[0])return;const u=String(l[0]).trim();let o=l[1]||u;const c=l[2]||"",g=l[3]||"";g.includes("Quelle BBCH")||g.includes("(G)")||g.includes("(D)")?o=g:(c.includes("Quelle BBCH")||c.includes("(G)")||c.includes("(D)"))&&(o=c),i.push({code:u,label:o,principal:l[4]!=null?Number(l[4]):null,secondary:null,definition:o,kind:null})}})):console.error("[BBCH Import] Keine bekannte Tabellenstruktur gefunden!"),console.log(`[BBCH Import] ${i.length} Einträge gelesen`)}finally{n.close()}t.exec("BEGIN TRANSACTION");try{if(w(t),t.exec("DELETE FROM lookup_bbch_stages"),i.length){const r=t.prepare(`INSERT OR REPLACE INTO lookup_bbch_stages
        (code, label, principal_stage, secondary_stage, definition, kind)
        VALUES (?, ?, ?, ?, ?, ?)`);for(const l of i)r.bind([l.code,l.label,l.principal,l.secondary,l.definition||null,l.kind]).step(),r.reset();r.finalize()}return x("lookup:bbch:lastImport",new Date().toISOString()),x("lookup:bbch:count",String(i.length)),t.exec("COMMIT"),{success:!0,count:i.length}}catch(r){throw t.exec("ROLLBACK"),console.error("BBCH-Lookup-Import fehlgeschlagen",r),r}}function ln(e={}){if(!t)throw new Error("Database not initialized");w(t);const a=(e.query||"").trim(),s=Math.min(Math.max(Number(e.limit)||10,1),50),n=Math.max(Number(e.offset)||0,0),i=(e.language||"").trim().toUpperCase(),r=i.length>0,l=[],u=t.selectValue("SELECT COUNT(*) FROM lookup_eppo_codes")||0;if(console.log(`[EPPO Search] Suche: "${a}", Sprache: "${i}", hasLanguageFilter: ${r}`),console.log(`[EPPO Search] Gesamt ${u} Einträge in lookup_eppo_codes`),a&&u>0){const T=`%${a.toUpperCase()}%`,S=t.selectValue("SELECT COUNT(*) FROM lookup_eppo_codes WHERE UPPER(name) LIKE ? OR UPPER(IFNULL(name_de, '')) LIKE ?",[T,T])||0;console.log(`[EPPO Search] ${S} Treffer für "${a}"`);const m=[];t.exec({sql:"SELECT code, name, language, name_de FROM lookup_eppo_codes LIMIT 3",callback:E=>{E&&m.push({code:E[0],name:E[1],lang:E[2],name_de:E[3]})}}),console.log("[EPPO Search] Beispiel-Einträge:",m)}if(r&&i==="DE")return un({query:a,limit:s,offset:n});const o=`
    SELECT
      code,
      name,
      dtcode,
  ...
  `,c="UPPER(IFNULL(language, '')) = ?",g=T=>{r&&T.push(i)};if(!a){const T=r?`WHERE ${c}`:"",S=[];g(S),S.push(s,n),t.exec({sql:`${o}
    ...
    `,callback:s=>{if(!s)return;const n=s[0]?String(s[0]).trim().toUpperCase():"",i=s[1]?String(s[1]).trim():null,r=s[2]!=null?Number(s[2]):0;e.push({language:n,label:i,count:r})}});const a=Number(t.selectValue(`SELECT COUNT(*) FROM (${Le()}) AS german_lookup`))||0;if(a>0){const s=e.find(n=>(n.language||"").toUpperCase()==="DE");if(s){const n=s.label?s.label.trim():"";s.label=n||"Deutsch",s.count=a}else e.push({language:"DE",label:"Deutsch",count:a})}return e.sort((s,n)=>{const i=l=>l===""?0:1,r=i(s.language||"")-i(n.language||"");return r!==0?r:(s.language||"").localeCompare(n.language||"")}),e.map(s=>{const n=s.language||"",i=s.label&&s.label.length?s.label:n||"Ohne Sprachcode";return{language:n,label:i,count:s.count}})}function Le(){return`
    SELECT
      code,
      name,
      dtcode,
  ...
  `}function cn(e={}){if(!t)throw new Error("Database not initialized");w(t);const a=(e.query||"").trim(),s=Math.min(Math.max(Number(e.limit)||10,1),50),n=Math.max(Number(e.offset)||0,0),i=[],r="ORDER BY label";if(!a){t.exec({sql:`
        SELECT code, label, principal_stage, secondary_stage, definition, kind
        FROM lookup_bbch_stages
        ${r}
        LIMIT ? OFFSET ?
      `,bind:[s,n],callback:c=>{i.push({code:c[0],label:c[1],principalStage:c[2]!=null?Number(c[2]):null,secondaryStage:c[3]!=null?Number(c[3]):null,definition:c[4]||null,kind:c[5]||null})}});const o=Number(t.selectValue("SELECT COUNT(*) FROM lookup_bbch_stages"))||0;return{rows:i,total:o}}const l=`%${a.toUpperCase()}%`,u=Number(t.selectValue("SELECT COUNT(*) FROM lookup_bbch_stages WHERE code LIKE ? OR UPPER(label) LIKE ? OR UPPER(IFNULL(definition, '')) LIKE ?",[l,l,l]))||0;return t.exec({sql:`
      SELECT code, label, principal_stage, secondary_stage, definition, kind
      FROM lookup_bbch_stages
      ...
      ORDER BY 
        ...
        label
      LIMIT ? OFFSET ?
    `,bind:[l,l,l,l,l,s,n],callback:o=>{i.push({code:o[0],label:o[1],principalStage:o[2]!=null?Number(o[2]):null,secondaryStage:o[3]!=null?Number(o[3]):null,definition:o[4]||null,kind:o[5]||null})}}),{rows:i,total:u}}function dn(){if(!t)throw new Error("Database not initialized");w(t);const e=Number(t.selectValue("SELECT COUNT(*) FROM lookup_eppo_codes"))||0,a=Number(t.selectValue("SELECT COUNT(*) FROM lookup_bbch_stages"))||0;return{eppo:{count:e,lastImport:U("lookup:eppo:lastImport")},bbch:{count:a,lastImport:U("lookup:bbch:lastImport")}}}})();
