...
`);try{await navigator.clipboard.writeText(g),N.success("Werte kopiert.")}catch{N.warning("Kopieren nicht möglich.")}}const ue=i=>({icon:'<i class="bi bi-palette"></i>',label:"Farbe",submenu:[{type:"swatchGrid",colors:It,current:i.color,onPick:c=>x(i,c),onLive:c=>dn(i,c)}]}),be=i=>({icon:'<i class="bi bi-flower1"></i>',label:"Kultur zuweisen",submenu:[{icon:'<i class="bi bi-x"></i>',label:"– keine –",action:()=>L(i,null)},...Qt.length?[{sep:!0}]:[],...Qt.map(c=>({icon:c.kultur===i.kultur?'<i class="bi bi-check2"></i>':"",label:`${c.kultur}${c.anbau?" ("+c.anbau+")":""}`,action:()=>L(i,c.kultur)}))]});function pe(i,c,g){lt(i._key);const{x:S,y:b}=sa(c),D=!!i.editing;We(S,b,[{icon:'<i class="bi bi-clipboard2-pulse"></i>',label:"Kulturführung öffnen",action:()=>lr(i)},{icon:'<i class="bi bi-pencil"></i>',label:"Umbenennen",action:()=>R(i)},be(i),ue(i),{sep:!0},{icon:'<i class="bi bi-arrow-clockwise"></i>',label:"Beete drehen +15°",keepOpen:!0,action:()=>Aa(i,15)},{icon:'<i class="bi bi-arrow-counterclockwise"></i>',label:"Beete drehen −15°",keepOpen:!0,action:()=>Aa(i,-15)},{icon:'<i class="bi bi-bounding-box"></i>',label:"Beete an Fläche ausrichten",action:()=>la(i)},{icon:'<i class="bi bi-grid-3x3-gap"></i>',label:i.bedsHidden?"Beete einblenden":"Beete ausblenden",action:()=>W(i)},{icon:'<i class="bi bi-bounding-box-circles"></i>',label:D?"Eckpunkte fertig":"Eckpunkte bearbeiten",action:()=>{D?Kt(i):ra(i)}},{sep:!0},{icon:'<i class="bi bi-copy"></i>',label:"Duplizieren",action:()=>P(i)},{icon:'<i class="bi bi-zoom-in"></i>',label:"Auf Fläche zoomen",action:()=>ia(i)},{icon:'<i class="bi bi-clipboard-data"></i>',label:"Werte kopieren",action:()=>T(i)},{icon:'<i class="bi bi-download"></i>',label:"Als GeoJSON exportieren",action:()=>B(i)},{sep:!0},{icon:'<i class="bi bi-trash"></i>',label:"Löschen",danger:!0,action:()=>Pa(i._key)}],g?`${i.name||"Fläche"} · Beet ${g}`:i.name||"Fläche")}function ee(i,c,g){const{x:S,y:b}=sa(g);We(S,b,[{icon:'<i class="bi bi-node-minus"></i>',label:"Eckpunkt löschen",disabled:i.latlngs.length<=3,action:()=>{i.latlngs.length<=3||(i.latlngs.splice(c,1),ot(i))}},{icon:'<i class="bi bi-check2"></i>',label:"Bearbeiten beenden",action:()=>Kt(i)}],`Eckpunkt ${c+1}`)}function Q(){const{sat:i,dop:c,osm:g}=l;if(!i||!g)return;let S=2;c&&q.hasLayer(c)?S=0:q.hasLayer(i)&&(S=1);const b=(S+1)%3;[i,c,g].forEach(D=>{D&&q.hasLayer(D)&&q.removeLayer(D)}),b===0?(i.addTo(q),c&&c.addTo(q),N.info("Karte: Luftbild BW – scharf (20 cm)")):b===1?(i.addTo(q),N.info("Karte: Satellit (weltweit)")):(g.addTo(q),N.info("Karte: Straßenkarte (OSM)"))}function te(i){const c=i.latlng,{x:g,y:S}=sa(i);We(g,S,[{icon:'<i class="bi bi-pencil-square"></i>',label:"Neue Fläche hier zeichnen",action:()=>{ht(!0),hi({latlng:c})}},{icon:'<i class="bi bi-crosshair"></i>',label:"Hierhin zentrieren",action:()=>q.panTo(c)},{sep:!0},{icon:'<i class="bi bi-arrows-fullscreen"></i>',label:"Alle Flächen anzeigen",disabled:!a.some(b=>b.latlngs?.length>=3),action:Da},{icon:'<i class="bi bi-layers"></i>',label:"Kartentyp wechseln (Luftbild BW / Satellit / OSM)",action:Q},{sep:!0},{icon:'<i class="bi bi-geo-alt"></i>',label:"Koordinaten kopieren",action:async()=>{try{await navigator.clipboard.writeText(`${c.lat.toFixed(6)}, ${c.lng.toFixed(6)}`),N.success("Koordinaten kopiert.")}catch{N.warning("Kopieren nicht möglich.")}}}],"Karte")}function he(i){return['<option value="">– Kultur –</option>'].concat(Qt.map(c=>{const g=`${c.kultur}${c.anbau?" ("+c.anbau+")":""}`;return`<option value="${v(c.kultur)}"${c.kultur===i?" selected":""}>${v(g)}</option>`})).join("")}function Se(i){const c=Ue(t.state.getState().gps?.points)||[];return['<option value="">– Standort –</option>'].concat(c.map(g=>`<option value="${v(g.id)}"${g.id===i?" selected":""}>${v(g.name||"")}</option>`)).join("")}function Ke(i){const c=f(i);return c?`<span class="acker-cropchip" title="Kultur"><span class="dot" style="background:${v(c.color||"#94a3b8")}"></span>${v(c.name)}</span>`:""}function H(i){if(i=Math.round(i||0),i<=0)return"0";let c=100;return i>=1e5?c=1e3:i>=1e4&&(c=500),"≈ "+V(Math.round(i/c)*c)}function fe(i,c){const g=i.result?.beds?.length||0,S=i.params.bedW+i.params.pathW;if(g<1||S<=0){N.warning("Erst Beete berechnen lassen.");return}const D=+(g*S/c-i.params.pathW).toFixed(2);if(D<.1){N.warning("Wegbreite ist größer als der gewünschte Abstand – erst Wegbreite verkleinern.");return}i.params.bedW=D,ot(i),N.success(`Bettbreite ${V(D,2)} m → ${i.result?.beds?.length||0} Beete.`)}function ze(){if(!F)return;let i=0,c=0,g=0,S=0;a.forEach(D=>{i+=D.result?.areaM2||0,c+=D.result?.beds?.length||0,g+=D.result?.bedMeters||0,S+=D.result?.plants||0});const b=(D,I)=>{const G=F.querySelector(D);G&&(G.textContent=I)};b('[data-t="area"]',V(i)+" m² · "+V(i/1e4,3)+" ha"),b('[data-t="beds"]',V(c)),b('[data-t="meters"]',V(g)+" m"),b('[data-t="plants"]',H(S))}function ke(i,c){const g=c.result||{},S=i.querySelector(".acker-stat");S&&(S.textContent=V(g.plants||0)+" Pfl.");const b=(I,G)=>{const ie=i.querySelector(`[data-r="${I}"]`);ie&&(ie.textContent=G)};b("area",V(g.areaM2||0)+" m² · "+V((g.areaM2||0)/1e4,3)+" ha"),b("pitch",V(c.params.bedW+c.params.pathW,2)+" m"),b("beds",V(g.beds?.length||0)),b("meters",V(g.bedMeters||0)+" m"),b("plants",H(g.plants||0));const D=g.beds||[];if(D.length){const I=G=>D.reduce((ie,le)=>ie+(G(le)||0),0)/D.length;b("rows-per-bed",V(I(G=>G.rows),1)),b("per-row",V(I(G=>G.perRow),1)),b("plants-per-bed",V(I(G=>G.plants),0)),b("bed-area",V(I(G=>G.areaM2),1)+" m²")}ze()}const Pt=["Jan.","Feb.","März","Apr.","Mai","Juni","Juli","Aug.","Sep.","Okt.","Nov.","Dez."];function j(i){if(!i)return"";const c=new Date(String(i).slice(0,10)+"T00:00:00");return isNaN(c.getTime())?"":`${c.getDate()}. ${Pt[c.getMonth()]}`}function se(i){const c=i?.ernteVon?j(i.ernteVon):"",g=i?.ernteBis||i?.ernteDatum?j(i.ernteBis||i.ernteDatum):"";return c&&g?`Ernte ${c}–${g}`:g?`Ernte ~${g}`:c?`Ernte ab ${c}`:""}function Ae(){const i=M('[data-role="acker-info"]');i&&(i.style.display="none")}function Gt(i){const c=M('[data-role="acker-info"]');if(!c)return;const{current:g,next:S}=fa(h(i.typ,i.id)),b=k(i.typ,i.id).filter(tt=>tt.status==="geplant").length,D=i.typ==="haus",I=i.area?`${V(i.area)} m²`:"",G=g?ka(g):"#94a3b8",ie=g?`<div class="ai-row"><span class="ai-dot" style="background:${v(G)}"></span>
      ...
      <div class="ai-actions">
      ...
      </div>`,c.style.display="block",c.querySelector('[data-ai="close"]')?.addEventListener("click",Ae),c.querySelector('[data-ai="kultur"]')?.addEventListener("click",()=>{Qe("app",tt=>({...tt,activeSection:"kultur"}));try{window.dispatchEvent(new CustomEvent("psm:openKultur",{detail:{typ:i.typ,id:String(i.id)}}))}catch{}}),c.querySelector('[data-ai="zoom"]')?.addEventListener("click",()=>{!D&&i.fieldRef?ia(i.fieldRef):i.latlng&&q.setView(i.latlng,Math.max(q.getZoom(),18))})}function Ye(){if(!_||!Z||!F)return;Z.style.display=a.length?"none":"block",F.style.display=a.length?"block":"none",_.innerHTML="";let i=0,c=0,g=0,S=0;a.forEach(b=>{i+=b.result?.areaM2||0,c+=b.result?.beds?.length||0,g+=b.result?.bedMeters||0,S+=b.result?.plants||0;const D=b._key===n,I=document.createElement("div");I.className="acker-field"+(D?" sel open":""),I.innerHTML=`
        ...
        <div class="acker-fbody">
          ...
          <div class="acker-res">
            ...
            <div class="r"><span>Pflanzen (geschätzt)</span><b data-r="plants">${H(b.result?.plants||0)}</b></div>
            <div class="sep"></div>
        ...
        </div>`,I.querySelector(".acker-fhead").addEventListener("click",le=>{le.target.classList.contains("acker-name")||(lt(b._key),Gt({typ:"acker",id:b.id,name:b.name,area:b.result?.areaM2||0,fieldRef:b}))}),I.querySelector(".acker-name").addEventListener("input",le=>{b.name=le.target.value,E(b)}),I.querySelectorAll("[data-k]").forEach(le=>{const ct=le.dataset.k;if(ct==="kultur"){le.addEventListener("input",et=>{b.kultur=et.target.value||null,b.eppoCode=Qt.find(tt=>tt.kultur===b.kultur)?.eppoCode||null,De(b),E(b)});return}if(ct==="standortId"){le.addEventListener("input",et=>{b.standortId=et.target.value||null,E(b)});return}le.addEventListener("input",et=>{if(ct==="angle"?b.params.angle=+et.target.value:b.params[ct]=parseFloat(et.target.value)||0,b.result=z(b.latlngs,b.params),De(b),ke(I,b),ct==="angle"){const tt=I.querySelector(".acker-angle-head b");tt&&(tt.textContent=b.params.angle+"°")}E(b)})}),I.querySelector('[data-act="align"]')?.addEventListener("click",()=>la(b)),I.querySelector('[data-act="calib"]')?.addEventListener("click",()=>{const le=Math.round(Number(I.querySelector("[data-calib]")?.value)||0);le>=1?fe(b,le):N.warning("Bitte die echte Beetzahl eingeben.")}),I.querySelector('[data-act="del"]').addEventListener("click",()=>Pa(b._key)),I.querySelector('[data-act="zoom"]').addEventListener("click",()=>ia(b)),I.querySelector('[data-act="dup"]').addEventListener("click",()=>P(b)),I.querySelector('[data-act="rot"]').addEventListener("click",()=>Aa(b,15)),I.querySelector('[data-act="pdf"]').addEventListener("click",()=>w(b));const ie=I.querySelector('[data-act="color"]');ie.addEventListener("input",le=>dn(b,le.target.value)),ie.addEventListener("change",le=>x(b,le.target.value)),_.appendChild(I)}),F.querySelector('[data-t="area"]').textContent=V(i)+" m² · "+V(i/1e4,3)+" ha",F.querySelector('[data-t="beds"]').textContent=V(c),F.querySelector('[data-t="meters"]').textContent=V(g)+" m",F.querySelector('[data-t="plants"]').textContent=V(S)}function lt(i){n=i,a.forEach(c=>De(c)),Ye(),setTimeout(()=>{const c=_?.querySelector(".acker-field.sel");c&&c.scrollIntoView({behavior:"smooth",block:"nearest"})},20)}async function Pa(i){const c=a.find(S=>S._key===i);if(!c)return;xe(c);const g=a.findIndex(S=>S._key===i);if(g>=0&&a.splice(g,1),n===i&&(n=null),Ye(),c.id&&ce()==="sqlite")try{await bl({id:c.id}),await it().catch(()=>{})}catch{}}let Be=null;function un(i,c){Ut||i._key!==n||(Be={fl:i,lastLL:c.latlng,moved:!1},q.dragging.disable(),q.getContainer().style.cursor="grabbing",X.DomEvent.stop(c))}function dr(i){if(!Be)return;const c=Be.fl;if(!Be.moved){c.bedsLayer&&(q.removeLayer(c.bedsLayer),c.bedsLayer=null);try{c.outline.setStyle({fillOpacity:.3,dashArray:"6 5"})}catch{}}const g=i.latlng.lat-Be.lastLL.lat,S=i.latlng.lng-Be.lastLL.lng;Be.lastLL=i.latlng,Be.moved=!0,c.latlngs=c.latlngs.map(b=>[b[0]+g,b[1]+S]);try{c.outline.setLatLngs(c.latlngs)}catch{}if((c.handles||[]).forEach((b,D)=>{try{b.setLatLng(c.latlngs[D])}catch{}}),c.label)try{c.label.setLatLng(c.outline.getBounds().getCenter())}catch{}}function pn(){if(!Be)return;const i=Be.fl,c=Be.moved;Be=null,q.dragging.enable(),q.getContainer().style.cursor="",c&&ot(i)}function Ma(i){if(me.length<3)return!1;const c=q.latLngToContainerPoint(X.latLng(me[0][0],me[0][1])),g=q.latLngToContainerPoint(i);return c.distanceTo(g)<=14}function Mt(i){const c=M('[data-role="acker-draw-stats"]');if(!c)return;const g=fu(i);c.textContent=`${me.length} Punkt${me.length===1?"":"e"}`+(g>0?` · ~${V(g)} m²`:"")}let Ut=!1,me=[],Xe=null,bt=[],Ca=0,fn=0,Oe=null;function mn(){return Oe||(Oe=document.createElement("div"),Oe.className="acker-draw-cur",Oe.innerHTML='<svg width="38" height="38" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="19" cy="19" r="9" stroke="#22c55e" stroke-width="2.5"/><line x1="19" y1="2" x2="19" y2="9" stroke="#22c55e" stroke-width="2.5" stroke-linecap="round"/><line x1="19" y1="29" x2="19" y2="36" stroke="#22c55e" stroke-width="2.5" stroke-linecap="round"/><line x1="2" y1="19" x2="9" y2="19" stroke="#22c55e" stroke-width="2.5" stroke-linecap="round"/><line x1="29" y1="19" x2="36" y2="19" stroke="#22c55e" stroke-width="2.5" stroke-linecap="round"/><circle cx="19" cy="19" r="2.5" fill="#22c55e"/></svg>',document.body.appendChild(Oe)),Oe}function gn(i){const c=mn();c.style.left=i.clientX+"px",c.style.top=i.clientY+"px",c.style.opacity="1"}function Ct(){Oe&&(Oe.style.opacity="0")}function ur(){Xe&&(q.removeLayer(Xe),Xe=null),bt.forEach(i=>q.removeLayer(i)),bt=[],me=[]}function ht(i){Ut=i,M('[data-role="acker-banner"]').style.display=i?"block":"none",M('[data-role="acker-draw"]').style.display=i?"none":"block";const c=q.getContainer();if(i){fn=Date.now()+250,c.style.cursor="none";const g=mn();g.style.opacity="0",c.addEventListener("mousemove",gn),c.addEventListener("mouseleave",Ct),q.on("mousemove",vi)}else c.style.cursor="",Oe&&(Oe.style.opacity="0"),c.removeEventListener("mousemove",gn),c.removeEventListener("mouseleave",Ct),q.off("mousemove",vi),ur()}function Vt(i){const c=i?[...me,[i.lat,i.lng]]:me;if(c.length<2){Xe&&(q.removeLayer(Xe),Xe=null);return}Xe?Xe.setLatLngs(c):Xe=X.polygon(c,{interactive:!1,className:"acker-draw-preview",color:"#22c55e",weight:2.5,fillColor:"#22c55e",fillOpacity:.18,dashArray:"6 5"}).addTo(q)}function pr(i,c){const g=X.circleMarker(i,{radius:c?7:5,color:"#fff",fillColor:c?"#16a34a":"#22c55e",fillOpacity:1,weight:2,interactive:c,bubblingMouseEvents:!1}).addTo(q);c&&(g.bindTooltip("Zum Schließen anklicken",{direction:"top"}),g.on("click",S=>{X.DomEvent.stop(S),me.length>=3&&bn()})),bt.push(g)}function hi(i){if(!Ut){Ae();return}if(!(Date.now()<fn)){if(Ma(i.latlng)){bn();return}me.push([i.latlng.lat,i.latlng.lng]),pr(i.latlng,me.length===1),Vt(),Mt(me)}}function vi(i){if(!Ut||!me.length)return;const c=Ma(i.latlng);if(Vt(c?void 0:i.latlng),bt[0])try{bt[0].setRadius(c?10:7),bt[0].setStyle({weight:c?3:2})}catch{}Mt(c?me:[...me,[i.latlng.lat,i.latlng.lng]])}function yi(){if(!me.length)return;me.pop();const i=bt.pop();i&&q.removeLayer(i),Vt(),Mt(me)}function bn(){if(me.length<3){N.warning("Mindestens 3 Punkte setzen.");return}const i={_key:"new-"+ ++Ca,id:null,name:"Fläche "+(a.length+1),kultur:null,eppoCode:null,standortId:null,color:It[a.length%It.length],latlngs:me.map(c=>c.slice()),params:{...uu(),angle:oa(me)},outline:null,bedsLayer:null,handles:[],result:{areaM2:0,beds:[],bedMeters:0,plants:0}};a.push(i),ht(!1),n=i._key,ot(i),E(i,!0)}async function ki(){const i=M('[data-role="acker-q"]').value.trim();if(i)try{const g=await(await fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&q="+encodeURIComponent(i))).json();g[0]?q.setView([+g[0].lat,+g[0].lon],18):N.info("Nichts gefunden.")}catch{N.warning("Suche nicht verfügbar.")}}async function Co(){if(zn){setTimeout(()=>q&&q.invalidateSize(),60);return}zn=!0;try{const[D]=await Promise.all([Dt(()=>import("./leaflet-src.BcflbDBd.js").then(I=>I.l),__vite__mapDeps([3,4])).then(async I=>(await Dt(()=>Promise.resolve({}),__vite__mapDeps([5])),I)),tu()]);X=D.default||D}catch(D){console.warn("[Acker] Karten-Bibliotheken konnten nicht geladen werden:",D),Z&&(Z.textContent="Karte konnte nicht geladen werden (offline?)."),zn=!1;return}q=X.map(ae,{doubleClickZoom:!1,zoomControl:!0,attributionControl:!0,zoomSnap:.25,zoomDelta:1,wheelPxPerZoomLevel:50,inertia:!0,inertiaDeceleration:2500,inertiaMaxSpeed:1500}).setView([47.818,8.976],17);const i=X.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{maxZoom:21,maxNativeZoom:19,attribution:"Tiles © Esri"}).addTo(q),c=X.tileLayer.wms("https://owsproxy.lgl-bw.de/owsproxy/ows/WMS_LGL-BW_ATKIS_DOP_20_C",{layers:"IMAGES_DOP_20_RGB",format:"image/png",transparent:!0,maxZoom:21,maxNativeZoom:20,attribution:"Luftbild © LGL-BW (dl-de/by-2-0)"}).addTo(q),g=X.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:"© OpenStreetMap"});l={sat:i,dop:c,osm:g},s=X.layerGroup(),C(),s.addTo(q),o=X.layerGroup().addTo(q);const S=q.createPane("edgeLabels");S.style.zIndex="650",S.style.pointerEvents="none";const b=X.DomUtil.create("div","acker-info");b.setAttribute("data-role","acker-info"),b.style.display="none",q.getContainer().appendChild(b),X.DomEvent.disableClickPropagation(b),X.DomEvent.disableScrollPropagation(b),q.on("click",hi),q.on("contextmenu",D=>{if(Ut){X.DomEvent.preventDefault?.(D.originalEvent||D),yi();return}te(D)}),q.on("mousemove",dr),q.on("mouseup",pn),document.addEventListener("mouseup",pn),q.on("zoomend",or),M('[data-role="acker-draw"]').addEventListener("click",()=>ht(!0)),M('[data-role="acker-export"]')?.addEventListener("click",y),M('[data-role="acker-export-pdf"]')?.addEventListener("click",$),M('[data-role="acker-finish"]').addEventListener("click",bn),M('[data-role="acker-cancel"]').addEventListener("click",()=>ht(!1)),M('[data-role="acker-go"]').addEventListener("click",ki),M('[data-role="acker-q"]').addEventListener("keydown",D=>{D.key==="Enter"&&ki()}),M('[data-role="ctrl-fit"]')?.addEventListener("click",Da),M('[data-role="ctrl-labels"]')?.addEventListener("click",()=>{d=!d,M('[data-role="ctrl-labels"]')?.classList.toggle("on",d),La()}),M('[data-role="ctrl-beds"]')?.addEventListener("click",()=>{p=!p,M('[data-role="ctrl-beds"]')?.classList.toggle("on",p),La()}),M('[data-role="ctrl-basemap"]')?.addEventListener("click",Q),document.addEventListener("keydown",D=>{Ut&&(D.key==="Backspace"&&(D.preventDefault(),yi()),D.key==="Enter"&&bn(),D.key==="Escape"&&ht(!1))}),await Io(),await wi(),await Bo(),setTimeout(()=>q.invalidateSize(),60)}async function Io(){if(ce()==="sqlite")try{Qt=(await Jr())?.rows||[]}catch{Qt=[]}}async function wi(){if(ce()!=="sqlite"){u=[],m=[];return}try{u=(await $r())?.rows||[]}catch{u=[]}try{m=(await Pn())?.rows||[]}catch{m=[]}}async function Bo(){if(ce()==="sqlite")try{const i=await Yr();for(const g of i?.rows||[]){const S={_key:"db-"+g.id,id:g.id,name:g.name,kultur:g.kultur,eppoCode:g.eppoCode,standortId:g.standortId,color:g.color||It[a.length%It.length],latlngs:g.latlngs||[],params:{bedW:g.bedW??1.2,pathW:g.pathW??.4,rowSp:g.rowSp??.5,inRowSp:g.inRowSp??.4,angle:g.angle??0},outline:null,bedsLayer:null,handles:[],edgeLabels:[],result:{areaM2:0,beds:[],bedMeters:0,plants:0}};S.result=await z(S.latlngs,S.params),a.push(S),De(S)}Ye();const c=a.find(g=>g.latlngs?.length);if(c&&q)try{q.fitBounds(X.polygon(c.latlngs).getBounds(),{maxZoom:19,padding:[30,30]})}catch{}}catch(i){console.warn("[Acker] Flächen laden fehlgeschlagen:",i)}}t.state.subscribe(i=>{if(i?.app?.activeSection==="acker"){if(!zn){Co();return}(async()=>(await wi(),La(),Ye(),setTimeout(()=>q&&q.invalidateSize(),60)))()}}),Ye()}function gu(){return`
  <style>
  ...
  <section class="calc-section">
    ...
    <div class="kb2-legend">
    ...
    </div>`,e.querySelectorAll(".kb2-row").forEach(F=>{const ae=F.dataset.ukey;F.querySelector(".kb2-label")?.addEventListener("click",()=>s&&s(ae)),F.addEventListener("contextmenu",O=>{O.preventDefault(),o&&o(ae,O.clientX,O.clientY)})})}const $u=[{art:"bewaesserung",label:"Gießen",icon:"bi-droplet"},{art:"mechanisch",label:"Hacken",icon:"bi-tools"},{art:"duengung",label:"Düngen",icon:"bi-flower1"},{art:"nuetzlinge",label:"Nützlinge",icon:"bi-bug"},{art:"chemisch_psm",label:"Pflanzenschutz",icon:"bi-droplet-half"},{art:"monitoring",label:"Kontrolle",icon:"bi-eye"},{art:"sonstiges",label:"Sonstiges",icon:"bi-three-dots"}],zu=["Jan.","Feb.","März","Apr.","Mai","Juni","Juli","Aug.","Sep.","Okt.","Nov.","Dez."];function Au(e,t){if(!(e instanceof HTMLElement))return;e.innerHTML=Pu();let a=[],n=[],r=[],s=[],o=[],l=null,d="plan",p=!1,u=!1;const m={};let h=null;const k=x=>e.querySelector(x),f=()=>k('[data-role="list"]'),y=()=>k('[data-role="detail"]'),w=()=>k('[data-role="kpis"]'),$=()=>k('[data-role="board-view"]'),C=()=>k('[data-role="flaechen-view"]'),M=()=>ce()==="sqlite",_=()=>{M()&&it().catch(()=>{})},Z=(x,L)=>x.filter(W=>W.flaecheTyp===L.typ&&String(W.flaecheId)===String(L.id)),F=x=>a.find(L=>_a(L)===x)||null,ae=(x,L=0)=>yo(x.color)||Yt[L%Yt.length];async function O(){if(a=await Ss(t),M()){try{n=(await $r())?.rows||[]}catch{n=[]}try{r=(await Pn())?.rows||[]}catch{r=[]}try{s=(await Jr())?.rows||[]}catch{s=[]}try{o=(await vl())?.rows||[]}catch{o=[]}if(!u){u=!0;try{const x=await Pi();x?.imported&&(r=(await Pn())?.rows||[],N.info(`${x.imported} Pflanzenschutz-Eintrag(e) übernommen.`),_())}catch{}}}!l&&a.length&&(l=_a(a[0])),Y(),z()}async function E(){if(M()){try{n=(await $r())?.rows||[]}catch{}try{r=(await Pn())?.rows||[]}catch{}}}async function z(){const x=l?F(l):null;if(!x||x.lat==null||x.lon==null)return;const L=_a(x);if(!m[L]){m[L]={loading:!0,weeks:[]};try{m[L]=await Lu(x.lat,x.lon)}catch{m[L]={weeks:[]}}l===L&&Te()}}function Y(){de(),d==="plan"?(C().style.display="none",$().style.display="block",Du($(),{units:a,anbau:n,mass:r,onSelect:x=>{l=x,re("flaechen"),z()},onContext:(x,L,W)=>za(x,L,W)})):($().style.display="none",C().style.display="grid",De(),Te()),e.querySelectorAll(".km-modebtn").forEach(x=>x.classList.toggle("active",x.dataset.mode===d))}function re(x){d=x,Y()}function de(){const x=w();if(!x)return;a.filter(P=>P.typ==="haus").length,a.filter(P=>P.typ==="acker").length;let L=0,W=null;a.forEach(P=>{const{current:B,next:T}=fa(Z(n,P));B&&L++,T?.pflanzDatum&&(!W||Ee(T.pflanzDatum)<Ee(W.pflanzDatum))&&(W=T)});const R=r.filter(P=>P.status==="geplant").length;x.innerHTML=`
      ${xe(String(a.length),"Flächen")}
      ${xe(String(L),"Kulturen aktiv")}
      ${xe(String(R),"Aufgaben offen")}
      ${xe(W?Ia(Wt(W.pflanzDatum)):"–","Nächste Pflanzung")}
      <button class="km-psm" data-role="psm-import" title="Bestehende Pflanzenschutz-Einträge übernehmen"><i class="bi bi-arrow-down-circle"></i><span>PSM übernehmen</span></button>`,x.querySelector('[data-role="psm-import"]')?.addEventListener("click",ot)}const xe=(x,L)=>`<div class="km-kpi"><div class="km-kpi-v">${x}</div><div class="km-kpi-l">${v(L)}</div></div>`;function De(){const x=f();if(!x)return;if(!a.length){x.innerHTML='<div class="km-empty"><i class="bi bi-geo-alt"></i><p>Noch keine Flächen.<br>Gewächshäuser unter Einstellungen, Freiland im Reiter „Karte".</p></div>';return}const L=a.filter(P=>P.typ==="haus"),W=a.filter(P=>P.typ==="acker"),R=(P,B)=>B.length?`<div class="km-grp">${v(P)}</div>`+B.map(Je).join(""):"";x.innerHTML=R("Gewächshäuser",L)+R("Freiland",W),x.querySelectorAll("[data-ukey]").forEach(P=>{P.addEventListener("click",()=>{l=P.dataset.ukey,De(),Te(),z()}),P.addEventListener("contextmenu",B=>{B.preventDefault(),za(P.dataset.ukey,B.clientX,B.clientY)})})}function Je(x,L){const W=_a(x),{current:R}=fa(Z(n,x));return`<div class="km-row${W===l?" sel":""}" data-ukey="${W}">
      ...
      </div>`};let B="";return L.length?B+=L.map(T=>P(T,!1)).join(""):B+='<div class="km-tasks-none"><i class="bi bi-check2-circle"></i> Nichts offen</div>',W.length&&(B+='<div class="km-done-h">Erledigt</div>'+W.map(T=>P(T,!0)).join("")),`<div class="km-tasks">${B}</div>`}function Wt(x){const L=new Date(String(x).slice(0,10)+"T00:00:00");return isNaN(L.getTime())?0:Wn(L).week}function ra(x){const L=new Date(String(x).slice(0,10)+"T00:00:00");return isNaN(L.getTime())?"":`${L.getDate()}. ${zu[L.getMonth()]}`}function Kt(x,L){if(!x)return"offen";const W=new Date(String(x).slice(0,10)+"T00:00:00");if(isNaN(W.getTime()))return"offen";const R=new Date;R.setHours(0,0,0,0);const P=Math.round((W.getTime()-R.getTime())/864e5);return P===0?"heute":P===1?"morgen":L?"überfällig":ra(x)}function La(x){if(!x||!x.weeks?.length)return x?.loading?"Wetter lädt…":"";const{year:L,week:W}=Wn(new Date),R=x.weeks.find(T=>T.year===L&&T.week===W)||x.weeks.find(T=>!T.isForecast);if(!R)return"";const P=R.tMaxAvg!=null?Math.round(R.tMaxAvg)+"°":"–",B=R.precipSum!=null?Math.round(R.precipSum)+" mm":"–";return`<i class="bi bi-cloud-sun"></i> Diese Woche: ${P} · ${B} Regen`}function or(x){const L=x.ernteVon?Ia(Wt(x.ernteVon)):null,W=x.ernteBis||x.ernteDatum,R=W?Ia(Wt(W)):null;return L&&R?` · Ernte ${L}–${R}`:R?` · Ernte ~${R}`:L?` · Ernte ab ${L}`:""}function dn(x){return!x||x.menge==null||x.menge===""?"":` · ${x.menge} ${x.einheit||"Pflanzen"}`}function ia(x,L){if(!x.length&&!L.length)return"";const W=new Date;let R=new Date(W.getFullYear(),W.getMonth()-1,1),P=new Date(W.getFullYear(),W.getMonth()+4,1);const B=H=>{if(!H)return;const fe=new Date(String(H).slice(0,10)+"T00:00:00");isNaN(fe.getTime())||(fe<R&&(R=new Date(fe.getFullYear(),fe.getMonth(),1)),fe>P&&(P=new Date(fe.getFullYear(),fe.getMonth(),1)))};x.forEach(H=>{B(H.pflanzDatum),B(H.ernteBis||H.ernteDatum),B(H.ernteVon)}),L.forEach(H=>B(H.planDatum||H.erledigtDatum));const T=wo(R,P),ue=T.length,be=`background-size:${(100/ue).toFixed(4)}% 100%`,pe=H=>H==null?null:(H*100).toFixed(2)+"%",ee=je(T,W.toISOString()),Q=ee!=null?`<div class="ks-today" style="left:${pe(ee)}"></div>`:"",te=T.map(H=>`<div class="ks-mo${H.y===W.getFullYear()&&H.m===W.getMonth()?" cur":""}">${ko[H.m]}</div>`).join("");let he="";x.forEach((H,fe)=>{const ze=je(T,H.pflanzDatum);let ke=je(T,H.ernteBis||H.ernteDatum||H.pflanzDatum);if(ze==null)return;(ke==null||ke<=ze)&&(ke=Math.min(1,ze+.5/ue));const Pt=ka(H,fe);he+=`<div class="ks-bar${H.status==="geplant"?" planned":""}" style="left:${pe(ze)};width:${((ke-ze)*100).toFixed(2)}%;--cc:${v(Pt)}"><span>${v(H.kultur||"")}</span></div>`;const j=je(T,H.ernteVon),se=je(T,H.ernteBis);j!=null&&se!=null&&se>j&&(he+=`<div class="ks-harvest" style="left:${pe(j)};width:${((se-j)*100).toFixed(2)}%"></div>`)});const Se={};L.forEach(H=>{(Se[H.art]=Se[H.art]||[]).push(H)});const Ke=Ud.filter(H=>Se[H]).map(H=>{const fe=Yn[H],ze=Se[H].map(ke=>{const Pt=ke.status==="erledigt"?ke.erledigtDatum||ke.planDatum:ke.planDatum||ke.erledigtDatum,j=je(T,Pt);return j==null?"":`<span class="ks-mk${ke.status==="erledigt"?" done":""}" title="${v(fe.label+(ke.notes?": "+ke.notes:""))}" style="left:${pe(j)};--mc:${fe.color}"></span>`}).join("");return`<div class="ks-row"><div class="ks-rl">${v(fe.label)}</div><div class="ks-track" style="${be}">${ze}${Q}</div></div>`}).join("");return`<div class="ks-wrap">
    ...
    </div>`}function Da(x){Qe("app",L=>({...L,activeSection:"acker"})),N.info("Karte geöffnet.")}async function ot(){if(!M()){N.warning("Keine Datenbank aktiv.");return}try{const x=await Pi();await E(),Y(),x?.imported?(N.success(`${x.imported} übernommen.`),_()):N.info(`Nichts Neues${x?.skipped?` (${x.skipped} nicht zuordenbar)`:""}.`)}catch{N.error("Übernahme fehlgeschlagen.")}}async function lr(x){const L=r.find(W=>W.id===x);if(L)try{await Ci({...L,status:"erledigt",erledigtDatum:L.erledigtDatum||dt()}),await E(),Y(),_()}catch{N.error("Speichern fehlgeschlagen.")}}async function Ie(x){try{await Mi({id:x}),await E(),Y(),_()}catch{N.error("Löschen fehlgeschlagen.")}}let ye=null;const jt=()=>{ye&&(ye.remove(),ye=null,document.removeEventListener("pointerdown",$a,!0))},$a=x=>{ye&&!ye.contains(x.target)&&jt()};function cr(x,L,W,R){if(jt(),ye=document.createElement("div"),ye.className="km-ctx",R){const B=document.createElement("div");B.className="km-ctx-t",B.textContent=R,ye.appendChild(B)}W.forEach(B=>{if(B.sep){const ue=document.createElement("div");ue.className="km-ctx-sep",ye.appendChild(ue);return}const T=document.createElement("button");T.className="km-ctx-i",T.innerHTML=`<i class="bi ${B.icon}"></i><span>${v(B.label)}</span>`,T.addEventListener("click",()=>{jt(),B.action?.()}),ye.appendChild(T)}),document.body.appendChild(ye);const P=ye.getBoundingClientRect();ye.style.left=Math.max(8,Math.min(x,window.innerWidth-P.width-8))+"px",ye.style.top=Math.max(8,Math.min(L,window.innerHeight-P.height-8))+"px",setTimeout(()=>document.addEventListener("pointerdown",$a,!0),0)}function za(x,L,W){const R=F(x);if(!R)return;const P=Z(n,R),{current:B}=fa(P);cr(L,W,[{icon:"bi-flower2",label:B?"Kultur bearbeiten":"Kultur setzen",action:()=>oa(R,B,"current",P.length)},{icon:"bi-plus-lg",label:"Nächste Kultur planen",action:()=>oa(R,null,"next",P.length)},{icon:"bi-list-check",label:"Aufgabe planen",action:()=>la(R,null,B)},{sep:!0},{icon:"bi-arrow-right-circle",label:"Fläche öffnen",action:()=>{l=x,re("flaechen"),z()}},{icon:"bi-map",label:"Auf Karte",action:()=>Da()}],R.name)}function We(){h&&(h.remove(),h=null)}function sa(x,L,W,R){We();const P=document.createElement("div");return P.className="kmodal-ov",P.innerHTML=`<div class="kmodal" role="dialog" aria-modal="true">
