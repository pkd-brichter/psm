 ...
 * @returns {number}
 */
export function bearing_deg(lat1, lng1, lat2, lng2) {
    const ret = wasm.bearing_deg(lat1, lng1, lat2, lng2);
    return ret;
}
 ...
 * @param {string} latlngs_json
 * @param {number} bed_w
 * @param {number} path_w
 * @param {number} row_sp
 * @param {number} in_row_sp
 * @param {number} angle_deg
 * @returns {string}
 */
export function compute_beds(latlngs_json, bed_w, path_w, row_sp, in_row_sp, angle_deg) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(latlngs_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.compute_beds(ptr0, len0, bed_w, path_w, row_sp, in_row_sp, angle_deg);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
...
export function haversine_m(lat1, lng1, lat2, lng2) {
    const ret = wasm.haversine_m(lat1, lng1, lat2, lng2);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('psm_geo_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
