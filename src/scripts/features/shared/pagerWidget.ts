...
export type PagerWidgetState =
  | { status: "hidden" }
  | { status: "disabled"; info?: string }
  ...
  | {
      ...
      canPrev: boolean;
      canNext: boolean;
      loadingDirection?: PagerDirection | null;
    };

export interface PagerWidget {
  update: (state: PagerWidgetState) => void;
  destroy: () => void;
  getState: () => PagerWidgetState;
}

export interface PagerWidgetOptions {
  onPrev: () => void;
  onNext: () => void;
...
export function createPagerWidget(
  target: Element | null,
  options: PagerWidgetOptions
): PagerWidget | null {
  if (!target) {
    return null;
  }
  ...
  let currentState: PagerWidgetState = { status: "hidden" };
  ...
  function renderDisabled(
    state: Extract<PagerWidgetState, { status: "disabled" }>
  ): void {
  ...
  function renderError(
    state: Extract<PagerWidgetState, { status: "error" }>
  ): void {
  ...
  function renderReady(
    state: Extract<PagerWidgetState, { status: "ready" }>
  ): void {
  ...
  function renderState(state: PagerWidgetState): void {
    switch (state.status) {
      case "hidden":
        renderHidden();
        break;
